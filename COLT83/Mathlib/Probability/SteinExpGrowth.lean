...
The Stein identities of `COLT83.Mathlib.Probability.SteinIdentity` assume a bounded derivative.
Here we relax this hypothesis to `C¹` functions `F` such that `|F x|` and `‖DF(x)‖` are bounded
by `A * exp (B * ‖x‖)`: such functions, and their products with linear forms, are integrable under
every Gaussian measure (Fernique's theorem, `IsGaussian.integrable_exp_mul_norm`), and the
one-dimensional identity only needs integrability (`integral_sub_mul_gaussianReal_of_integrable`).
