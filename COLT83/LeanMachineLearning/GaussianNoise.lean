...
The general ingredients are `hasSubgaussianMGF_fun_id_gaussianReal` (a centered real Gaussian
is sub-Gaussian, in `COLT83.Mathlib.Probability.GaussianMGF`) and
`HasCondDistrib.hasCondSubgaussianMGF_of_const` (in `COLT83.LeanMachineLearning.CondSubgaussian`).
-/
