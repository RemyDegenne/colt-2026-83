...
be measurable. For $n\ge0$ and a history $h = (x_t,y_t)_{t< n}$ of the first $n$ rounds define
$D_n^\rho(h) := \prod_{t<n}\rho(x_t,y_t)$, a measurable function of $h$ (an empty product,
hence $1$, for $n = 0$).
\end{definition}
...
\lean{Learning.IsAlgEnvSeq.map_history_eq_withDensity_env}\leanok
...
laws of the history $(x_t,y_t)_{t< n}$ under $\mathrm{stationaryEnv}(\kappa)$, resp.\
...
\emph{Base case.} $\mathcal{H}_0$ is a one-point space, so $\Prob^{(0)} = \Prob_0^{(0)}$ is the
Dirac mass at the empty history (LML \texttt{hasLaw\_history\_zero}) and
$D_0^\rho = 1$ is the empty product.
...
\textbf{Lean remark.} \texttt{Learning.envDensity ρ n : Hist Unit 𝓐 𝓨 n → ℝ≥0∞} is the
product $\prod_{t<n}\rho(x_t,y_t)$ over \texttt{Fin n}, matching LML's histories
\texttt{history O X Y n : Hist Unit 𝓐 𝓨 n} (rounds \texttt{Unit × 𝓐 × 𝓨}: there are no
observations), with the recursion \texttt{envDensity\_snoc}; the law
identity \texttt{map\_history\_eq\_withDensity\_env} is proved by induction on $n$ from the
composition-product recursion of the law of the history. The kernel-level identity
...
\lean{Learning.LinearBandit.noiseHistLaw, Learning.LinearBandit.histKernel, Learning.LinearBandit.histKernel_apply, Learning.IsAlgEnvSeq.map_history_eq_histKernel, Learning.IsAlgEnvSeq.hasLaw_history_histKernel, Learning.LinearBandit.pairKernel, Learning.LinearBandit.pairKernel_apply, Learning.IdentAlg.IsRun.hasLaw_history_out_pairKernel, Learning.LinearBandit.IsPAC.le_measureReal_pairKernel, Learning.LinearBandit.IsPAC.integral_simpleRegret_pairKernel_le, Maiti2026Power.expRegret, Maiti2026Power.stronglyMeasurable_expRegret}\leanok
