...
Helmert matrices with every linear-algebra step isolated (the Lean proof replaces the explicit
basis by a basis-free reduction to a Gaussian vector on the span, see the remarks in
Section~\ref{sec:width_multitask}).

\section{Upper bounds}
\label{sec:width_upper}

\begin{proposition}[The width is at most $d$]\label{prop:width_le_d}
\lean{Maiti2026Power.gw_le_card, Maiti2026Power.IsGOptimalDesign.gwMat_le_card}\leanok
...
\lean{Maiti2026Power.gw_le_sqrt_log_ncard, Maiti2026Power.IsGOptimalDesign.gwMat_le_sqrt_log_ncard}\leanok
\uses{def:width, thm:kiefer_wolfowitz}
...
\lean{Maiti2026Power.regionalizedGw, Maiti2026Power.gwMat_le_regionalizedGw, Maiti2026Power.regionalizedGw_le, Maiti2026Power.regionalizedGw_nonneg, Maiti2026Power.regionalizedGw_le_gwMat}\leanok
...
\textbf{Lean remark.} In Lean the regions are simply a family of sets
$\mathcal R : \kappa \to \mathcal P(\R^d)$ indexed by a finite type $\kappa$, and
$\gw(A;\mathcal R) := \sup_{i \in \kappa}\gwidth{R_i}{A}$ (\texttt{regionalizedGw}); the
action set $\Xs$ does not appear in the definition, and the partition property (disjointness,
covering) is never needed: the statements below only use $R_i \subseteq \Xs$, and each region
nonempty (and finite, with at most $p$ elements). The inner
...
\lean{Maiti2026Power.IsGOptimalDesign.regionalizedGw_le_sqrt_log}\leanok
...
\lean{Maiti2026Power.gwMat_unitBall, Maiti2026Power.one_le_sqrt_diag_mul_sqrt_diag_inv, Maiti2026Power.le_gwMat_unitBall, Maiti2026Power.le_gw_unitBall, Maiti2026Power.div_sqrt_three_le_gw_unitBall, Maiti2026Power.gw_unitBall_le_card}\leanok
\uses{def:width}
...
\[
  \frac{d}{\sqrt 3} \le \sqrt{\frac{2}{\pi}}\,d \le \gw(\ball_d) \le d .
\]
More precisely, for every $A \in \PD$, $\gwidth{\ball_d}{A} \ge \sqrt{2/\pi}\,d/\sqrt{\tr A}$.
\end{proposition}

\begin{proof}
\leanok
\uses{prop:width_le_d, lem:designset_basic, lem:gauss_norm_lower, lem:gauss_linear_image, lem:width_matrix_wd}
The upper bound is Proposition~\ref{prop:width_le_d} ($\ball_d$ being spanning). For every
$v \in \R^d$, $\sup_{x\in\ball_d}\ip{x}{v} = \norm{v}$ (Cauchy--Schwarz, with equality at
$x = v/\norm{v}$ when $v \ne 0$), so $\gwidth{\ball_d}{A} = \E\norm{Y}$ with
$Y := A^{-1/2}g \sim \Normal(0, A^{-1})$ (Lemma~\ref{lem:gauss_linear_image}). Fix $A \in \PD$
and apply Lemma~\ref{lem:gauss_norm_lower} with the weights $c_i := \sqrt{A_{ii}/\tr A}$
($\sum_i c_i^2 = 1$):
$\E\norm Y \ge \sqrt{2/\pi}\sum_i \sqrt{A_{ii}}\sqrt{(A^{-1})_{ii}}/\sqrt{\tr A}$. Now
$\sqrt{A_{ii}}\sqrt{(A^{-1})_{ii}} \ge 1$ for every $i$: by Cauchy--Schwarz,
$1 = \ip{e_i}{e_i} = \ip{A^{1/2}e_i}{A^{-1/2}e_i} \le \norm{A^{1/2}e_i}\norm{A^{-1/2}e_i}
= \sqrt{A_{ii}}\sqrt{(A^{-1})_{ii}}$. Hence
$\gwidth{\ball_d}{A} \ge \sqrt{2/\pi}\,d/\sqrt{\tr A}$. For $A \in \designset(\ball_d)$,
Lemma~\ref{lem:designset_basic} gives $\tr A \le \sup_{x\in\ball_d}\norm{x}^2 = 1$, so
$\gwidth{\ball_d}{A} \ge \sqrt{2/\pi}\,d$ for every admissible $A$, and the infimum
$\gw(\ball_d)$ is at least $\sqrt{2/\pi}\,d \ge d/\sqrt 3$ (as $\pi \le 6$).
\end{proof}

\begin{lemma}[Well-separated points in the whitened set]\label{lem:whitened_separated_points}
\lean{exists_pos_weight_sq_dist_ge_of_isotropic, exists_separated_of_isotropic, Maiti2026Power.sum_mul_inner_sqrt_inv_sq}\leanok
...
\textbf{Lean remark.} The lemma is formalized in library generality
(\texttt{Maiti2026Power/Mathlib/Analysis/InnerProductSpace/IsotropicSeparated.lean}): for a weighted
family $(w_i, u_i)$ of a $d$-dimensional inner product space in \emph{isotropic position},
$\sum_i w_i\ip{u_i}{v}^2 = \norm v^2$ for all $v$ ($w_i \ge 0$, $\sum_i w_i = 1$), and every
$m$, there are $m$ points $u_{i_1},\dots,u_{i_m}$ with positive weights and
$\norm{u_{i_k} - u_{i_l}}^2 \ge d + 1 - m$ for $k \ne l$ (\texttt{exists\_separated\_of\_isotropic};
no hypothesis on $m$ or $d$). The greedy step uses an orthonormal basis $(f_j)$ of
$V_k^\perp$ and Bessel's inequality $\norm{u - t_i}^2 \ge \sum_j\ip{u - t_i}{f_j}^2 = \sum_j\ip{u}{f_j}^2$
instead of the projection $Q_k$. The isotropy of the whitened design,
$\sum_x\lambda_x\ip{\Sigma^{-1/2}x}{v}^2 = \norm v^2$, is \texttt{sum\_mul\_inner\_sqrt\_inv\_sq}.

\begin{proposition}[Lower bound $\sqrt{d\log d}$; secondary]\label{prop:width_lower_sqrt_dlogd}
\lean{Maiti2026Power.sqrt_log_le_gw}\leanok
\uses{def:width}
...
(Lean: \texttt{Maiti2026Power.sqrt\_log\_le\_gwMat}, for every design $\lambda$ with $A(\lambda)\succ0$).
...
\begin{corollary}[Lower bound $\sqrt{d\log d}$]\label{cor:width_lower_sqrt_dlogd}
\uses{prop:width_lower_sqrt_dlogd}
Let $\Xs \subset \R^d$ be a spanning action set with $d \ge 6$. Then
$\gw(\Xs) \ge \frac{1}{8\sqrt 2}\sqrt{d\log d}$.
\end{corollary}
\begin{proof}
\uses{prop:width_lower_sqrt_dlogd}
For $d \ge 6$, $\lfloor d/2\rfloor \ge (d-1)/2 \ge \sqrt d$ (as $(d-1)^2 \ge 4d$ for $d \ge 6$), so
$\log\lfloor d/2\rfloor \ge \frac12\log d$ and Proposition~\ref{prop:width_lower_sqrt_dlogd} gives
the claim.
\end{proof}
...
\lean{Maiti2026Power.intrinsicGw}\leanok
...
\lean{Maiti2026Power.multitaskSet, Maiti2026Power.oneHot, Maiti2026Power.multitaskSet_eq_range, Maiti2026Power.ncard_multitaskSet, Maiti2026Power.supportFn_multitaskSet, Maiti2026Power.norm_le_of_mem_multitaskSet}\leanok
...
partition $(B_j)_j$ of the coordinates and the sizes $d_j$. The Lean development uses the sigma
type; \texttt{oneHot d κ} is the element with ones at the positions $(k_j)_j = \kappa$, so that
$\Xs$ is the range of \texttt{oneHot} (\texttt{multitaskSet\_eq\_range}), $\abs\Xs = \prod_j d_j$,
and the support function is $\sup_{x\in\Xs}\ip{x}{y} = \sum_j\max_{k\in B_j}y_k$
(\texttt{supportFn\_multitaskSet}), the identity on which the width computation rests.

\textbf{Lean remark.} The Helmert matrices and the explicit orthonormal basis of
Lemma~\ref{lem:multitask_basis} are not formalized: the Lean proof of
Proposition~\ref{prop:width_multitask} avoids any explicit basis of the span through the
general reduction \texttt{intrinsicGw\_le\_gaussianWidth} (the Lean form of
Lemma~\ref{lem:multitask_width_reduction}), which only needs the span of $\Xs$
(Lemma~\ref{lem:multitask_basis}, first part), a design matrix $\Sigma$ of $\Xs$
(Lemma~\ref{lem:multitask_design_cov}) and a positive semidefinite matrix $S$ with range in
$\Span\Xs$ inverting $\Sigma$ on $\Span\Xs$; the block-centering maps $P_j$ play the role
of $H_{d_j}H_{d_j}^\top$. Definition~\ref{def:helmert} and Lemma~\ref{lem:helmert_props} are
kept as the paper's route.
...
\lean{Maiti2026Power.mem_span_multitaskSet_iff, Maiti2026Power.finrank_span_multitaskSet_add, Maiti2026Power.single_sub_single_mem_span, Maiti2026Power.blockSum}\leanok
...
(Lean: only the description $\Span\Xs = \{v : \text{all block sums of } v \text{ are equal}\}$
and $\dim\Span\Xs = d - m + 1$ are formalized, the latter by rank--nullity for the map
$v \mapsto (\sum_{i\in B_j}v_i - \sum_{i\in B_1}v_i)_j$; the matrix $U$ is not needed.)
...
\lean{Maiti2026Power.unifDesignMatrix, Maiti2026Power.unifDesignMatrix_mem_designSet, Maiti2026Power.toEuclideanLin_unifDesignMatrix_muVec, Maiti2026Power.toEuclideanLin_unifDesignMatrix_blockCenter, Maiti2026Power.blockCenter, Maiti2026Power.muVec, Maiti2026Power.kappa}\leanok
...
(Lean: the formalized statements are the two eigen-relations $\Sigma\mu = \kappa\mu$ and
$\Sigma v = v/d_j$ for $v \in R_j$, i.e.\ for $v = P_jw$ the block-centering of a vector $w$
on the block $j$; they are obtained by splitting the sum over $\prod_jB_j$ according to one
coordinate, \texttt{sum\_pi\_split}.)
...
\lean{Maiti2026Power.intrinsicGw_le_gaussianWidth, Maiti2026Power.intrinsicGw_eq_gw_image, Maiti2026Power.spanBasisMatrix, Maiti2026Power.mtL, Maiti2026Power.mtS, Maiti2026Power.multivariateGaussian_mtS, Maiti2026Power.toEuclideanLin_unifDesignMatrix_mtL_mtL, Maiti2026Power.mtL_apply_mk}\leanok
...
(Lean: the general statement \texttt{intrinsicGw\_le\_gaussianWidth} says that for any
$\Sigma \in \designset(\Xs)$ and any $S \succeq 0$ with range in $V = \Span\Xs$ and
$\Sigma S = \mathrm{id}$ on $V$, one has $\gw_{\mathrm{int}}(\Xs) \le \E\sup_{x\in\Xs}\ip{x}{G}$
with $G \sim \Normal(0,S)$: with $U$ the coordinate matrix of the standard orthonormal basis
of $V$, $A := U^\top\Sigma U$ is a positive definite design matrix of $U^\top\Xs$ with
$A^{-1} = U^\top SU$, and $\Normal(0,U^\top SU)$ is the image of $\Normal(0,S)$ by $U^\top$
while $\ip{U^\top x}{U^\top y} = \ip{x}{y}$ for $x \in V$. For the multi-task set, $\Sigma$ is
the uniform design and $S = L^2$ with $L v := \kappa^{-3/2}\ip{\mu}{v}\mu + \sum_j\sqrt{d_j}P_jv$
(\texttt{mtL}), whose coordinates in the block $j$ are $\ip{w_j}{v} + \sqrt{d_j}v_k$ for a fixed
vector $w_j$ (\texttt{mtL\_apply\_mk}); the block maxima of $Lg$, $g\sim\Normal(0,I_d)$, are
then $\ip{w_j}{g} + \sqrt{d_j}\max_{k\in B_j}g_k$, and the first term has mean zero.)
...
\lean{Maiti2026Power.integral_iSup_mtL_le, ciSup_const_add_const_mul}\leanok
...
(Lean: in the formalization the centered maximum appears directly as
$\max_k(g_k - \bar g) = \max_kg_k - \bar g$ with $\E\bar g = 0$, so no change of
variables is needed; \texttt{integral\_iSup\_mtL\_le} is the statement
$\E\max_{k\in B_j}(Lg)_k \le \sqrt{2d_j\log d_j}$.)
...
\lean{Maiti2026Power.intrinsicGw_multitaskSet_le}\leanok
...
\lean{Maiti2026Power.width_separation}\leanok
