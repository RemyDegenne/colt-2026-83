...
\lean{Maiti2026Power.bayesParam, Maiti2026Power.bayesObs, Maiti2026Power.bayesHist, Maiti2026Power.bayesKernel, Maiti2026Power.bayesJoint, Maiti2026Power.map_bayesParam_stdGaussian, Maiti2026Power.map_bayesHist_stdGaussian}\leanok
...
\textbf{Lean remark.} In Lean the Bayesian model is built directly on a standard Gaussian pair
$(g,\eta)$ of $\R^d\times\R^T$ (\texttt{stdGaussian}$\times$\texttt{stdGaussian}): the reward
vector is $\theta = \tau\Sigma_T^{-1/2}g$ (\texttt{bayesParam}, whose law is
$\Normal(0,\tau^2\Sigma_T^{-1})$ by \texttt{map\_bayesParam\_stdGaussian}), the observations are
$y_t = \ip{x_t}{\theta} + \eta_t$ (\texttt{bayesObs}), the history is
$((x_t,y_t))_{t<T}$ (\texttt{bayesHist}) and the recommendation is drawn from the output rule of
the algorithm applied to that history (\texttt{bayesKernel}); the joint law of $((g,\eta),\rec)$
is \texttt{bayesJoint}. Conditionally on $g$, the law of the history is the fixed-design law of
Lemma~\ref{lem:fixed_design_law} with reward vector $\theta$
(\texttt{map\_bayesHist\_stdGaussian}), which is how the PAC property enters
(\texttt{measureReal\_lt\_simpleRegret\_bayes\_le}).
...
\lean{Maiti2026Power.bayesW₁, Maiti2026Power.bayesW₂, Maiti2026Power.bayesY₁, Maiti2026Power.bayesParam_sub_smul_leastSquares, Maiti2026Power.bayesW₁_comp_adjoint_bayesY₁_add, Maiti2026Power.indepFun_bayesW_bayesY}\leanok
...
\lean{Maiti2026Power.integral_inner_bayesParamClm_bayesJoint, Maiti2026Power.integral_inner_bayesW_bayesJoint}\leanok
...
\lean{Maiti2026Power.le_integral_simpleRegret_bayesJoint, Maiti2026Power.integral_inner_bayesLSclm_le, Maiti2026Power.integral_supportFn_bayesParam, Maiti2026Power.integral_supportFn_toEuclideanLin_lsMatrix}\leanok
...
\lean{Maiti2026Power.bayesC_identity}\leanok
...
(In Lean only the algebraic identity $\tau - c(\tau+1) = (\sqrt2-1)/2$ is isolated,
\texttt{bayesC\_identity}; the numerical bound $0.207$ is used inline in the proof of
...
\lean{Maiti2026Power.exists_singular_instances, Maiti2026Power.exists_singular_instances_of_zero_mem, Maiti2026Power.exists_singular_instances_of_forall, Maiti2026Power.exists_ne_zero_inner_eq_zero_of_not_posDef, Maiti2026Power.diffSup_pos_of_inner_eq_zero, Maiti2026Power.simpleRegret_smul_add_simpleRegret_neg_smul}\leanok
...
singular; if $T = 0$ (so that $\Sigma_0 = 0$), assume moreover that $0 \in \Xs$ (more generally,
it suffices that every direction $v$ orthogonal to the design, $\ip{x_t}{v} = 0$ for all $t$, is
orthogonal to some point of $\Xs$; for $T \ge 1$ this holds with the point $x_0$). Let
...
In general: the singularity of $\Sigma_T$ gives $v \ne 0$ with $\ip{x_t}{v} = 0$ for all $t$
(for $T = 0$ any $v \ne 0$ works, as $d \ge 1$), and by assumption $\ip{y_0}{v} = 0$ for some
$y_0 \in \Xs$ ($y_0 = x_0$ if $T \ge 1$, $y_0 = 0$ if $0 \in \Xs$); if $\ip\cdot v$ were constant
on $\Xs$ its value would be $\ip{y_0}{v} = 0$, and we conclude as before.
...
probability measure on $\Xs$. For $T = 0$ the additional hypothesis ($0 \in \Xs$, or a
direction condition) cannot be dropped: if $\Xs = \{x_0\}$ with $x_0 \ne 0$ (a spanning action set when $d = 1$),
...
\begin{lemma}[A PAC design is nonsingular]\label{lem:lower_nonadaptive_posdef}
\lean{Maiti2026Power.posDef_of_isFixedDesign_of_isPAC_of_forall, Maiti2026Power.posDef_of_isFixedDesign_of_isPAC, Maiti2026Power.posDef_of_isFixedDesign_of_isPAC_of_zero_mem}\leanok
\uses{def:fixed_design, def:pac, def:arm_set}
Let $\Xs$ be spanning, $\eps > 0$ and $\delta < 1/2$. If a fixed-design algorithm with design
$x_0,\dots,x_{T-1} \in \Xs$ is $(\eps,\delta)$-PAC on $\Xs$, and either $T \ge 1$ or
$0 \in \Xs$ (more generally, every direction orthogonal to the design is orthogonal to some
point of $\Xs$), then $\Sigma_T \succ 0$.
\end{lemma}

\begin{proof}
\leanok
\uses{lem:singular_design_fails, lem:fixed_design_law}
Otherwise Lemma~\ref{lem:singular_design_fails} gives $\theta_\pm$ under which $(y,\rec)$ has
the same law and $r(\rec,\theta_+) + r(\rec,\theta_-) = 4\eps$ pointwise, so that
$\Prob_{\theta_+}(r \le \eps) + \Prob_{\theta_-}(r \le \eps) \le 1$, while the PAC property
gives $2(1-\delta) > 1$.
\end{proof}

\begin{lemma}[The Bayesian step]\label{lem:lower_nonadaptive_bayes_step}
\lean{Maiti2026Power.gwMat_le_of_isFixedDesign_of_isPAC}\leanok
\uses{def:fixed_design, def:pac, def:width_matrix}
Let $\Xs$ be compact and nonempty, $\eps \ge 0$ and $\delta \in (0, 1/10]$. If a fixed-design
algorithm with design $x_0,\dots,x_{T-1} \in \Xs$ and $\Sigma_T \succ 0$ is $(\eps,\delta)$-PAC
on $\Xs$, then
\[
  0.12\,\gwidth{\Xs}{\Sigma_T} \le \eps .
\]
\end{lemma}

\begin{proof}
\leanok
\uses{def:bayes_prior, lem:fixed_design_law, lem:pac_expected_regret, lem:width_symm, lem:width_matrix_wd, lem:gauss_linear_image, lem:bayes_regret_lower_opt}
Write $w_T := \gwidth{\Xs}{\Sigma_T} \ge 0$.

\emph{Step 1: upper bound on the Bayesian regret.} Let $\tau := \sqrt2 - 1$ and consider the
...
\emph{Step 2: lower bound on the Bayesian regret.} By Lemma~\ref{lem:bayes_regret_lower_opt},
$\E[r(\rec,\theta)] \ge 0.207\, w_T$.

\emph{Step 3: arithmetic.} Combining, $(0.207 - 2\tau\delta)\, w_T \le \eps$. Since
...
\lean{Maiti2026Power.le_budget_of_isFixedDesign_of_isPAC}\leanok
...
\begin{proof}
\leanok
\uses{lem:lower_nonadaptive_posdef, lem:lower_nonadaptive_bayes_step, lem:width_empirical}
\emph{Step 1: $\Sigma_T$ is positive definite}, by Lemma~\ref{lem:lower_nonadaptive_posdef}
(with $T \ge 1$, $\Xs$ spanning and $\delta \le 1/10 < 1/2$). Write
$w_T := \gwidth{\Xs}{\Sigma_T} \ge 0$.

\emph{Step 2: the Bayesian step.} Lemma~\ref{lem:lower_nonadaptive_bayes_step} gives
$0.12\, w_T \le \eps$.

\emph{Step 3: from $w_T$ to $\gw(\Xs)$.} By Lemma~\ref{lem:width_empirical},
