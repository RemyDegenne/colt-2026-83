...
expected maximum are all absent from Mathlib; they are formalized in this project in
\texttt{Maiti2026Power/Mathlib/Probability/\{SteinReal, SteinIdentity, GaussianInterpolation,
SudakovFernique, GaussianMaxLower\}.lean} and
\texttt{Maiti2026Power/Mathlib/Analysis/\{SpecialFunctions, InnerProductSpace\}/LogSumExp.lean}.

\textbf{Lean remark (organisation).} The Lean development is coordinate-free where possible.
Stein's identity is proved for the standard Gaussian measure of a finite-dimensional inner
product space $E$ in the form $\E[\ip{a}{g}F(g)] = \E[DF(g)\,a]$
(\texttt{integral\_inner\_mul\_stdGaussian}, from the one-dimensional identity by Fubini along
the coordinates of an orthonormal basis), and for $X\sim\Normal(0,S)$ in the form
$\E[\ip{a}{X}F(X)] = \E[DF(X)(Sa)]$ (\texttt{integral\_inner\_mul\_multivariateGaussian}); the
version used by the interpolation is \texttt{integral\_fderiv\_apply\_stdGaussian}:
$\E[DF(Lg)(L'g)] = \E[\sum_k D^2F(Lg)(Le_k, L'e_k)]$ for linear maps $L, L' : E'\to E$ and an
orthonormal basis $(e_k)$ of $E'$. The interpolation $Z_t = \sqrt t\,X + \sqrt{1-t}\,Y$ with
$X, Y$ independent is realized as $Z_t = \sqrt t\,L_1 g + \sqrt{1-t}\,L_0 g$ for a single
standard Gaussian vector $g$ of $E'$ (\texttt{gaussianInterp}); for the Sudakov--Fernique
inequality, $E' = \R^{\iota\sqcup\iota}$, $L_1 g = S_X^{1/2}(g\circ\mathrm{inl})$ and
$L_0 g = S_Y^{1/2}(g\circ\mathrm{inr})$ (\texttt{blockInl}, \texttt{blockInr}), so that the
derivative of $\psi(t) = \E F(Z_t)$ is
$\E[\sum_k D^2F(Z_t)(Z_te_k, Z_t'e_k)]$ with the cross terms vanishing block by block
(\texttt{hasDerivAt\_integral\_comp\_gaussianInterp}, \texttt{sum\_apply\_gaussianInterp\_block}).
The soft-max is \texttt{logSumExp β (EuclideanSpace.basisFun ι ℝ)}: the coordinate soft-max
\texttt{Real.logSumExp} of \texttt{Maiti2026Power/Mathlib/Analysis/SpecialFunctions/LogSumExp.lean}
(defined on $\R^m$, with its Hessian \texttt{Real.fderiv\_fderiv\_logSumExp\_single}) composed
with the coordinates, as an instance of the general log-sum-exp smoothing of a maximum of linear
forms of \texttt{Maiti2026Power/Mathlib/Analysis/InnerProductSpace/LogSumExp.lean}.
...
\lean{ProbabilityTheory.integral_sub_mul_gaussianReal, ProbabilityTheory.integral_mul_gaussianReal_zero_one_of_hasDerivAt}\leanok
...
(In Lean the identity is proved for every $\Normal(\mu, v)$, $v \ge 0$, in the form
$\E[(\xi-\mu)f(\xi)] = v\,\E[f'(\xi)]$, for differentiable $f$ with bounded derivative.)
\end{lemma}
...
\lean{ProbabilityTheory.integral_inner_mul_stdGaussian, ProbabilityTheory.integral_inner_mul_multivariateGaussian, ProbabilityTheory.integral_fderiv_apply_stdGaussian}\leanok
\uses{def:pg_gaussian_vector}
...
\lean{ProbabilityTheory.gaussianInterp, ProbabilityTheory.hasDerivAt_integral_comp_gaussianInterp, ProbabilityTheory.continuousOn_integral_comp_gaussianInterp}\leanok
\uses{def:pg_gaussian_vector}
...
\lean{Real.logSumExp, Real.softmax, Real.softmax_nonneg, Real.sum_softmax, Real.ciSup_le_logSumExp, Real.logSumExp_le_ciSup_add, Real.contDiff_logSumExp, Real.hasFDerivAt_logSumExp, Real.norm_fderiv_logSumExp_le, Real.lipschitzWith_logSumExp, Real.fderiv_fderiv_logSumExp_apply, Real.fderiv_fderiv_logSumExp_single, Real.norm_fderiv_fderiv_logSumExp_le, logSumExp, softmax, ciSup_inner_le_logSumExp, logSumExp_le_ciSup_inner_add, hasFDerivAt_logSumExp, norm_gradient_logSumExp_le, fderiv_fderiv_logSumExp_apply, norm_fderiv_fderiv_logSumExp_le, ProbabilityTheory.fderiv_fderiv_logSumExp_basisFun_single, ProbabilityTheory.sum_mul_fderiv_fderiv_logSumExp_basisFun}\leanok
...
\lean{ProbabilityTheory.sudakov_fernique}\leanok
\uses{def:pg_gaussian_vector}
...
(Lean: \texttt{sudakov\_fernique}, for the measures $\Normal(0,S_X)$, $\Normal(0,S_Y)$ on
\texttt{EuclideanSpace ℝ ι} with $S_X, S_Y$ positive semidefinite and the increment condition
written as $(S_X)_{ii} + (S_X)_{jj} - 2(S_X)_{ij} \le (S_Y)_{ii} + (S_Y)_{jj} - 2(S_Y)_{ij}$.)
\end{theorem}
...
\lean{ProbabilityTheory.mul_gaussianPDFReal_le_measureReal_Ici}\leanok
...
\begin{proof}
\leanok
(The Lean proof integrates $\frac{d}{dx}\big[-\varphi(x)/x\big] = \varphi(x)(1 + x^{-2})$ over
$[t,\infty)$: $\varphi(t)/t = \int_t^\infty\varphi(x)(1+x^{-2})\,dx \le (1+t^{-2})\Prob(\xi\ge t)$,
which is the claim. The argument below is the one of the outline.)
...
\lean{ProbabilityTheory.integral_max_pi_gaussianReal, ProbabilityTheory.integral_posPart_gaussianReal_zero_one}\leanok
...
(Lean: for two distinct coordinates of a product of standard Gaussians, via
$\E[\xi^+] = (2\pi)^{-1/2}$, \texttt{integral\_posPart\_gaussianReal\_zero\_one}.)
\end{lemma}
...
\lean{le_ciSup}\leanok
Let $(\xi_i)_{i\ge1}$ be real random variables with $\E\abs{\xi_i}<\infty$. For $1\le m\le m'$,
$\E[\max_{i\le m}\xi_i] \le \E[\max_{i\le m'}\xi_i]$.
(In Lean this is used inline: $\max(\xi_i,\xi_j) \le \sup_k\xi_k$ pointwise, \texttt{le\_ciSup},
and \texttt{integral\_mono}.)
\end{lemma}
...
\lean{ProbabilityTheory.sqrt_log_div_four_le_integral_iSup_pi_gaussianReal_of_le}\leanok
...
\lean{ProbabilityTheory.sqrt_log_div_four_le_integral_iSup_pi_gaussianReal}\leanok
