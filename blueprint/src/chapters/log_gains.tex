...
\lean{Maiti2026Power.regionCandidates, Maiti2026Power.regionAlg, Maiti2026Power.start_regionAlg}\leanok
\uses{def:bai_alg, def:regionalized_width, thm:kiefer_wolfowitz, thm:rounding, def:least_squares, lem:approx_argmax_selector, def:median_elimination, cor:median_elimination_linear, lem:composition, lem:phased}
...
\textbf{Lean remark.} The region algorithm is the phased algorithm (Lemma~\ref{lem:phased})
\texttt{regionAlg}, obtained by prepending to Median Elimination on the state $(c, S)$
(Definition~\ref{def:median_elimination}, \texttt{PhasedAlg.cons}) a first phase of length $T_1$
which plays the design $x$ and whose observations $y$ set the state to
$(\,(s_i(\hat\theta(y)))_{i \le K},\ \mathrm{Fin}\,K\,)$ (\texttt{regionCandidates}); the regions
need only cover $\Xs$ (they need not be disjoint). The fixed design of phase~(1) is obtained
from the existence statement of Theorem~\ref{thm:rounding} by choice, in
Corollary~\ref{cor:log_gains_exists}. The recommendation is the surviving candidate
(\texttt{MedianElim.meOut}) and the budget is
$T_1 + N_{\mathrm{ME}}(K,\eps/2,\delta/2)$ (\texttt{start\_regionAlg}).

\begin{lemma}[Width of a region under a scalar Loewner bound]\label{lem:log_gains_region_width_bound}
\lean{Maiti2026Power.gwMat_le_of_smul_le}\leanok
...
(Formalized for $c = T/4$, the case of the rounding theorem.)
...
\lean{Maiti2026Power.one_sub_le_measureReal_regionCandidates, Maiti2026Power.region_phase1_arith}\leanok
...
(In Lean the statement is about the noise of the first phase: the observations are
$\ip{x_t}{\theta} + \eta_t$ with $\eta \sim \Normal(0, I_{T_1})$, and the conclusion holds with
probability at least $1 - \delta'$ over $\eta$ for any design with $\Sigma_{T_1} \succ 0$,
$\norm{x}^2_{\Sigma_{T_1}^{-1}} \le \sigma^2$ on $\Xs$ and
$2\gwidth{R_i}{\Sigma_{T_1}} + 2\sigma\sqrt{2\cG\log(1/\delta')} \le \eps'$ for all $i$;
the arithmetic below is \texttt{region\_phase1\_arith}.)
...
\begin{lemma}[Covering a finite set by small regions]\label{lem:cover_regions}
\lean{Set.exists_cover_ncard_le}\leanok
A finite set of $m \ge d \ge 1$ elements is covered by $d$ nonempty subsets of at most
$\lfloor m/d\rfloor + 1$ elements each.
\end{lemma}

\begin{proof}
\leanok
Enumerate the set as $y_0, \dots, y_{m-1}$ and let the $i$-th region be
$\{y_k : k \equiv i \pmod d\}$; it is nonempty since $i < d \le m$ and has at most
$\lfloor m/d\rfloor + 1$ elements.
\end{proof}

\begin{theorem}[Theorem~6 of the paper]\label{thm:log_gains}
\lean{Maiti2026Power.isPAC_regionAlg, Maiti2026Power.regionGood, Maiti2026Power.logGains_budget_arith}\leanok
\uses{def:region_algorithm, def:pac}
Let $\Xs$ be a finite spanning action set, $\eps\in(0,1]$, $\delta\in(0,1)$.
\begin{enumerate}
  \item[(i)] For every family $\mathcal R = (R_1,\dots,R_K)$ of nonempty subsets covering
        $\Xs$, the region
        algorithm of Definition~\ref{def:region_algorithm} is $(\eps,\delta)$-PAC on $\Xs$,
        with budget $T_1 + N_{ME}(K,\eps/2,\delta/2)$.
  \item[(ii)] Let $m := \abs\Xs$, $K := d$, and let $\mathcal R$ be a family of
        $d$ nonempty subsets covering $\Xs$, of size at most $p := \lfloor m/d\rfloor + 1$
        (it exists since $m \ge d$, Lemma~\ref{lem:cover_regions}).
...
\begin{corollary}[Theorem~6 of the paper, existential form]\label{cor:log_gains_exists}
\lean{Maiti2026Power.exists_isPAC_of_finite}\leanok
\uses{thm:log_gains, def:pac}
Let $\Xs$ be a finite spanning action set with $m := \abs\Xs$, $\eps \in (0,1]$ and
$\delta \in (0,1)$. There exist a budget
$T \le C_{\mathrm{reg}}\, d\,(\log((m/d)_+) + \log(4/\delta))/\eps^2$, with
$C_{\mathrm{reg}} = 1281 + 6C_{ME} = 97281$, and an identification algorithm with budget $T$ that is
$(\eps,\delta)$-PAC on $\Xs$.
\end{corollary}
\begin{proof}
\leanok
\uses{thm:log_gains, lem:me_budget, lem:cover_regions, thm:kiefer_wolfowitz, thm:rounding}
Take the region algorithm of Theorem~\ref{thm:log_gains}(ii); its budget is bounded by (ii)
with $C_{ME} = 16000$ from Lemma~\ref{lem:me_budget}. (In Lean the case $d = 0$ is handled
separately, as in Corollary~\ref{cor:upper_exists}.)
\end{proof}

\begin{proof}
\leanok
\uses{lem:region_phase1, cor:median_elimination_linear, lem:composition, lem:me_budget, prop:width_partition, lem:phased, lem:cover_regions}
\textbf{Lean.} (i) is \texttt{isPAC\_regionAlg}, by the phase-by-phase analysis of
Lemma~\ref{lem:phased} with the good states \texttt{regionGood}: after the first phase, the
surviving set is $\mathrm{Fin}\,K$ and some candidate is $\eps/2$-optimal
(Lemma~\ref{lem:region_phase1}); each round of Median Elimination preserves the goodness
(Lemma~\ref{lem:me_round}) with the target value $\max_x\ip{x}{\theta} - \eps/2$; the final
surviving candidate has regret at most $\eps/2 + \sum_\ell \eps_\ell(\eps/2) \le \eps$. The
failure probabilities $\delta/2 + \sum_\ell \delta_\ell(\delta/2) \le \delta$ add up over the phases.
(ii) is the arithmetic \texttt{logGains\_budget\_arith} with $p = \lfloor m/d\rfloor + 1 \le 2(m/d)_+$.
...
(ii) \emph{Phase (1).} By Proposition~\ref{prop:width_partition} with $p = \lfloor m/d\rfloor + 1$,
$w_{\mathcal R}^2 \le 2d\log p$. Now $\lfloor m/d\rfloor \ge 1$ since $m \ge d$, so
$p \le 2\lfloor m/d\rfloor \le 2 (m/d)_+$ and $\log p \le \log((m/d)_+) + \log 2$.
...
\lean{Learning.LinearBandit.IsRun.klDiv_map_out_le_sum_integral, Learning.LinearBandit.IsRun.abs_measureReal_sub_le_of_sum_le, Learning.LinearBandit.IsRun.abs_measureReal_sub_le_of_sq_le}\leanok
...
\textbf{Lean remark.} All the events used below depend on the recommendation only, so the Lean
statement is for events of $\rec$: \texttt{klDiv\_map\_out\_le\_sum\_integral} bounds the
divergence between the laws of the outputs of two runs by $\KL(\theta,\theta')$ (data processing
on top of the divergence decomposition), and
\texttt{abs\_measureReal\_sub\_le\_of\_sum\_le} turns any upper bound $K$ on
$\KL(\theta,\theta')$ into $\abs{\Prob_\theta(E) - \Prob_{\theta'}(E)} \le \sqrt{K/2}$, in the
two-sided form. \texttt{abs\_measureReal\_sub\_le\_of\_sq\_le} is the special case
$K = TC/2$ of a uniform bound $\ip{x}{\theta-\theta'}^2 \le C$ on $\Xs$, which is what the
hypercubes use; the multi-task and $m$-set arguments need the data-dependent form, whose $K$
involves the expected number of pulls of a coordinate.

\begin{lemma}[Three-point inequality]\label{lem:log_gains_three_point}
\lean{Learning.LinearBandit.IsRun.one_sub_le_measureReal_add_of_sq_le}\leanok
\uses{def:bai_alg}
...
\lean{Learning.LinearBandit.integral_simpleRegret_le_of_measureReal_le}\leanok
...
\textbf{Lean remark.} What the four lower bounds below actually use is the pathwise
inequality of the proof, in the contrapositive direction: if the recommendation of
$\mathsf{Alg}$ is $\eps_*$-optimal with probability at least $1-\delta$ on the instance
$\theta$ and $0 \le r(\rec,\theta) \le Z$ on $\Xs$ with $\eps_* \le Z$, then
\[
  \E_\theta\big[r(\rec,\theta)\big] \le \eps_* + (Z - \eps_*)\,\delta ,
\]
which for $Z = 10\eps_*$ reads $\E_\theta[r] \le \eps_*(1 + 9\delta)$. This is the Lean
statement \texttt{integral\_simpleRegret\_le\_of\_measureReal\_le}. Averaging it over the
family and comparing with a lower bound $b\eps_*$ on the average expected regret gives
$\delta \ge (b-1)/9$ directly, so no bad instance needs to be exhibited (and
\texttt{Finset.exists\_le\_of\_sum\_le} is not needed).
...
\lean{Maiti2026Power.mtSum, Maiti2026Power.mtEps, Maiti2026Power.mtParam, Maiti2026Power.mtParam0, Maiti2026Power.sum_mtEps, Maiti2026Power.inner_mtParam, Maiti2026Power.supportFn_mtParam, Maiti2026Power.inner_mtParam0_sub_mtParam, Maiti2026Power.mtParam0_update, Maiti2026Power.inner_mtParam_nonneg}\leanok
...
In Lean the instances are indexed by the arms themselves, in the form
$\kappa : \prod_j B_j$ of Definition~\ref{def:multitask_set}: \texttt{mtParam}$(d,\eps,\kappa)$
has the value $10\eps_j$ at the position $\kappa_j$ of the block $j$, and the alternative
\texttt{mtParam0}$(d,\eps,\kappa,j)$ is the same vector with the block $j$ zeroed. The relaxed set
$\tilde{\Xs}$ and the sets $\Xs^{(j)}$ are not needed: the alternative of the block $j$ is a
reward vector like any other (it is not required to lie in $\Xs$), and it depends on $\kappa$
only through the coordinates $\ell \ne j$ (\texttt{mtParam0\_update}).
...
\textbf{Lean remark.} The four parts are carried out inside the proof of
Theorem~\ref{thm:lower_multitask} rather than as separate declarations, since they all speak
about the same fixed run measures: (i) is the pathwise identity
$\sum_{i\in B_j}\rec_i = \sum_{i\in B_j}x_t[i] = 1$ integrated, (ii) is
Lemma~\ref{lem:two_point_pinsker} in its data-dependent form applied to
$\theta = \texttt{mtParam0}(d,\eps,\kappa,j)$ and
$\theta' = \texttt{mtParam}(d,\eps,\kappa^{j\to i})$, and
(iv) is Cauchy--Schwarz (Lemma~\ref{lem:sum_sqrt_cs}) over the block.

\begin{lemma}[Cauchy--Schwarz for square roots]\label{lem:sum_sqrt_cs}
\lean{Maiti2026Power.sum_sqrt_le_sqrt_card_mul_sum}\leanok
For nonnegative reals $(a_k)_{k \in K}$ indexed by a finite set $K$,
$\sum_{k\in K}\sqrt{a_k} \le \sqrt{\abs K\sum_{k\in K}a_k}$.
\end{lemma}

\begin{proof}
\leanok
Cauchy--Schwarz applied to the vectors $(1)_{k}$ and $(\sqrt{a_k})_k$ gives
$\big(\sum_k\sqrt{a_k}\big)^2 \le \abs K\sum_k a_k$; take square roots.
\end{proof}

\begin{lemma}[Averaging over one block]\label{lem:multitask_block_reindex}
\lean{Fintype.sum_sum_update}\leanok
\uses{def:multitask_set}
Let $j \le m$ and let $g$ be a real function of the arms $\kappa \in \prod_\ell B_\ell$ of the
multi-task set. Writing $\kappa^{j\to i}$ for $\kappa$ with its $j$-th coordinate replaced by
$i \in B_j$,
\[
  \sum_{\kappa}\sum_{i\in B_j} g\big(\kappa^{j\to i}\big) = d_j \sum_\kappa g(\kappa) .
\]
\end{lemma}

\begin{proof}
\leanok
The map $(\kappa, i) \mapsto (\kappa^{j\to i}, \kappa_j)$ is an involution of
$\big(\prod_\ell B_\ell\big)\times B_j$, so the left-hand side, which is the sum of
$g(\kappa^{j\to i})$ over all pairs $(\kappa,i)$, equals the sum of $g(\kappa)$ over all pairs,
namely $d_j\sum_\kappa g(\kappa)$. This replaces the bijection
$\Xs^{(j)}\times B_j \to \Xs$ of Definition~\ref{def:multitask_instances}, which would require
naming the set $\Xs^{(j)}$ of arms with an empty block.
\end{proof}

\begin{theorem}[Lower bound for multi-task bandits]\label{thm:lower_multitask}
\lean{Maiti2026Power.le_of_isPAC_multitaskSet}\leanok
...
$\Prob_{\theta_{\tilde x}}\big(r(\rec,\theta_{\tilde x}) > \eps\big) \ge 0.4$.
\end{theorem}

\begin{corollary}[PAC lower bound for multi-task bandits]\label{cor:lower_multitask_pac}
\lean{Maiti2026Power.multitaskSet_lt_budget_of_isPAC}\leanok
\uses{thm:lower_multitask, def:pac}
Let $\Xs$ be the multi-task action set (block sizes $d_j \ge 2$) and $\eps > 0$. An
$(\eps,\delta)$-PAC algorithm on $\Xs$ with budget $T \ge 1$ and $\delta < 0.4$ has
$T > S_d^2/(20000\,\eps^2)$.
\end{corollary}
\begin{proof}
\leanok
\uses{thm:lower_multitask}
If $T \le S_d^2/(20000\eps^2)$, Theorem~\ref{thm:lower_multitask} gives an instance
$\theta_{\tilde x}$ with $\Prob_{\theta_{\tilde x}}(r(\rec,\theta_{\tilde x}) > \eps) \ge 0.4 > \delta$,
contradicting the PAC property.
\end{proof}

\begin{proof}
\leanok
\uses{lem:multitask_block_average, lem:fail_prob_from_expected_value, def:multitask_instances, lem:multitask_block_reindex, lem:sum_sqrt_cs}
...
\textbf{Lean remark.} The Lean statement \texttt{le\_of\_isPAC\_multitaskSet} is the
contrapositive form: an $(\eps,\delta)$-PAC algorithm with $T \le S_d^2/(20000\eps^2)$ has
$\delta \ge 2/5$. It is obtained by comparing the upper bound $5.36\,\eps$ on the average value
with the lower bound $9\eps(1-\delta)$ that Lemma~\ref{lem:fail_prob_from_expected_value}
(PAC form) gives on every instance, using $Z(\theta_{\tilde x}) = 10\eps$; the averaging over
the block uses Lemma~\ref{lem:multitask_block_reindex}.
\end{proof}
...
\begin{definition}[Boxes with two values per coordinate]\label{def:cube_set}
\lean{Maiti2026Power.cubeSet, Maiti2026Power.cubeVec, Maiti2026Power.cubeSet_eq_range, Maiti2026Power.cubeSet_finite, Maiti2026Power.isCompact_cubeSet, Maiti2026Power.norm_le_of_mem_cubeSet, Maiti2026Power.signVec, Maiti2026Power.signVecZero, Maiti2026Power.signVecZero_update, Maiti2026Power.inner_smul_signVecZero_sub, Maiti2026Power.supportFn_cubeSet, Maiti2026Power.hammingCard, Maiti2026Power.simpleRegret_smul_signVec, Maiti2026Power.hypercubePM_eq_cubeSet, Maiti2026Power.hypercube01_eq_cubeSet}\leanok
\uses{def:arm_set, def:regret}
Let $u_- < u_+$ be two reals. The \emph{box} they define is the action set
\[
  \cubeset(u) := \{x \in \R^d : x_i \in \{u_-, u_+\}\ \text{ for all } i \le d\},
\]
a finite --- hence compact --- set of $2^d$ points, whose elements are indexed by the sign
patterns $s \in \{-,+\}^d$ through $c(s)_i := u_{s_i}$. Both hypercubes are boxes:
$\{-1,1\}^d$ is the box of $(u_-,u_+) = (-1,1)$ and $\{0,1\}^d$ that of $(u_-,u_+) = (0,1)$.
For a sign pattern $s$ let $\sigma(s) \in \{-1,1\}^d$ be the vector with $\sigma(s)_i = +1$ if
$s_i = +$ and $-1$ otherwise, and let $\sigma(s)^{\setminus j}$ be $\sigma(s)$ with the
coordinate $j$ replaced by $0$. For $c > 0$,
\[
  \max_{x\in\cubeset(u)}\ip{x}{c\,\sigma(s)} = \sum_{i\le d}\max\big(u_+c\sigma(s)_i, u_-c\sigma(s)_i\big)
  = c\sum_{i\le d}u_{s_i}\sigma(s)_i ,
\]
and for $x \in \cubeset(u)$ the simple regret is $c(u_+-u_-)$ times the Hamming distance to the
maximizer $c(s)$:
\[
  r\big(x, c\,\sigma(s)\big) = c\,(u_+ - u_-)\,\#\{i \le d : x_i \ne u_{s_i}\} ,
\]
so $0 \le r \le c(u_+-u_-)d$ on $\cubeset(u)$ and $Z(c\,\sigma(s)) = c(u_+-u_-)d$.
\end{definition}

\begin{lemma}[Lower bound on a box]\label{lem:lower_cube}
\lean{Maiti2026Power.le_of_isPAC_cubeSet}\leanok
\uses{def:cube_set, def:pac}
Let $\cubeset(u)$ be a box in $\R^d$, let $\eps > 0$, let $M$ satisfy
$\abs{u_-} \le M$ and $\abs{u_+} \le M$, and let $c > 0$ be normalized by
$c\,(u_+-u_-)\,d = 10\eps$. Every $(\eps,\delta)$-PAC identification algorithm on
$\cubeset(u)$ with budget $T$ satisfying $T\,(cM)^2 \le 1/4$ has $\delta \ge 1/6$.
\end{lemma}

\begin{proof}
\leanok
\uses{lem:log_gains_three_point, lem:fail_prob_from_expected_value, def:cube_set}
The hard instances are the $2^d$ reward vectors $c\,\sigma(s)$, $s \in \{-,+\}^d$.

\emph{One coordinate.} Fix $j \le d$ and a sign pattern $s$, and let $s'$ be $s$ with the sign
of the coordinate $j$ flipped. The vector $\theta^0 := c\,\sigma(s)^{\setminus j}$ is the same
for $s$ and for $s'$, and $\theta^0 - c\,\sigma(s) = -c\,\sigma(s)_je_j$, so for every
$x \in \cubeset(u)$,
$\ip{x}{\theta^0 - c\,\sigma(s)}^2 = c^2x_j^2 \le c^2M^2 =: C$, and likewise with $s'$. Since
$\rec \in \cubeset(u)$ takes only the two values $u_\pm$ in the coordinate $j$, the event
$\{\rec_j \ne u_{s_j}\}$ is $E := \{\rec_j = u_-\}$ when $s_j = +$ and $E^c$ when $s_j = -$, so
Lemma~\ref{lem:log_gains_three_point} gives
\[
  \Prob_{c\sigma(s)}(\rec_j \ne u_{s_j}) + \Prob_{c\sigma(s')}(\rec_j \ne u_{s'_j})
  \ge 1 - 2\sqrt{TC/4} \ge \frac12 ,
\]
using $T(cM)^2 \le 1/4$.

\emph{Averaging.} The map $s \mapsto s'$ is an involution of $\{-,+\}^d$, so summing the
display over the $2^d$ sign patterns gives
$2\sum_s\Prob_{c\sigma(s)}(\rec_j\ne u_{s_j}) \ge 2^{d}/2$: the average over $s$ of
$\Prob_{c\sigma(s)}(\rec_j\ne u_{s_j})$ is at least $1/4$. Summing over $j \le d$ and using the
Hamming form of the regret (Definition~\ref{def:cube_set}),
\[
  \frac{1}{2^d}\sum_s\E_{c\sigma(s)}\big[r(\rec, c\sigma(s))\big]
  = c(u_+-u_-)\sum_{j\le d}\frac{1}{2^d}\sum_s\Prob_{c\sigma(s)}(\rec_j\ne u_{s_j})
  \ge c(u_+-u_-)\frac d4 = \frac{10\eps}{4} = 2.5\,\eps .
\]

\emph{Conclusion.} On every instance $0 \le r \le Z(c\sigma(s)) = 10\eps$, so
Lemma~\ref{lem:fail_prob_from_expected_value} (PAC form) gives
$\E_{c\sigma(s)}[r] \le \eps + 9\eps\delta$. Averaging over $s$ and comparing with $2.5\eps$
yields $2.5 \le 1 + 9\delta$, i.e.\ $\delta \ge 1/6$.
\end{proof}

\begin{theorem}[Lower bound for the hypercube $\{-1,1\}^d$]\label{thm:lower_hypercube_pm}
\uses{def:bai_alg, def:pac, def:regret, def:cube_set}
...
$s \in \{-1,1\}^d$ with $\Prob_{\theta_s}(r(\rec,\theta_s) > \eps) \ge 1/6$.
\end{theorem}

\begin{proof}
\uses{lem:lower_cube, lem:fail_prob_from_expected_value, def:cube_set}
$\{-1,1\}^d$ is the box of $(u_-,u_+) = (-1,1)$, with $M = 1$ and
$c = 5\eps/d$ (so that $c(u_+-u_-)d = 10\eps$) and $\theta_s = c\,\sigma(s)$; the budget
assumption is $T(cM)^2 = 25\eps^2T/d^2 \le 1/4$. The average regret bound is the display of the
proof of Lemma~\ref{lem:lower_cube}, and Lemma~\ref{lem:fail_prob_from_expected_value} with
$\eps_* = \eps$ and $b = 2.5$ gives an instance with failure probability at least
$1.5/9 = 1/6$.
\end{proof}

\begin{corollary}[PAC lower bound for the hypercube $\{-1,1\}^d$]\label{cor:lower_hypercube_pm_pac}
\lean{Maiti2026Power.hypercubePM_lt_budget_of_isPAC}\leanok
\uses{lem:lower_cube, def:pac}
Let $\eps > 0$. An $(\eps,\delta)$-PAC algorithm on $\{-1,1\}^d$ with budget $T \ge 1$ and
$\delta < 1/6$ has $T > d^2/(100\,\eps^2)$.
\end{corollary}
\begin{proof}
\leanok
\uses{lem:lower_cube}
If $T \le d^2/(100\eps^2)$, Lemma~\ref{lem:lower_cube} applied to the box of $(-1,1)$ with
$M = 1$ and $c = 5\eps/d$ gives $\delta \ge 1/6$, a contradiction. (For $d = 0$ the bound reads
$0 < T$, which holds since $T \ge 1$.)
\end{proof}
...
$\Prob_{\theta_s}(r(\rec,\theta_s) > \eps) \ge 1/6$.
\end{theorem}

\begin{proof}
\uses{lem:lower_cube, lem:fail_prob_from_expected_value, def:cube_set}
$\{0,1\}^d$ is the box of $(u_-,u_+) = (0,1)$, with $M = 1$ and $c = 10\eps/d$ (so that
$c(u_+-u_-)d = 10\eps$); the budget assumption is $T(cM)^2 = 100\eps^2T/d^2 \le 1/4$. The rest
is as for Theorem~\ref{thm:lower_hypercube_pm}: the vertex maximizing $\ip{x}{\theta_s}$ is
$\indic\{s = 1\}$ and the regret is $\frac{10\eps}{d}$ times the Hamming distance to it.
\end{proof}

\begin{corollary}[PAC lower bound for the hypercube $\{0,1\}^d$]\label{cor:lower_hypercube_01_pac}
\lean{Maiti2026Power.hypercube01_lt_budget_of_isPAC}\leanok
\uses{lem:lower_cube, def:pac}
Let $\eps > 0$. An $(\eps,\delta)$-PAC algorithm on $\{0,1\}^d$ with budget $T \ge 1$ and
$\delta < 1/6$ has $T > d^2/(400\,\eps^2)$.
\end{corollary}
\begin{proof}
\leanok
\uses{lem:lower_cube}
As for Corollary~\ref{cor:lower_hypercube_pm_pac}, with $c = 10\eps/d$.
\end{proof}
...
\lean{Maiti2026Power.mSet, Maiti2026Power.msParam, Maiti2026Power.msVec, Maiti2026Power.msVec_mem_mSet, Maiti2026Power.mSet_nonempty, Maiti2026Power.norm_le_of_mem_mSet, Maiti2026Power.inner_msParam, Maiti2026Power.inner_msParam_sub_insert, Maiti2026Power.inner_msParam_nonneg, Maiti2026Power.supportFn_msParam}\leanok
\uses{def:arm_set, def:regret}
...
\lean{Finset.sum_powersetCard_succ_sum_mem}\leanok
\uses{def:msets}
...
\textbf{Lean remark.} The Lean statement is the unnormalized identity
$\sum_{\abs S = m}\sum_{i\in S}F(S\setminus\{i\},i) = \sum_{\abs B = m-1}\sum_{i\notin B}F(B,i)$
(the bijection alone, proved with \texttt{Finset.sum\_nbij'} on the two $\Sigma$-types); the
normalization $n\binom{d}{m-1} = m\binom dm$ is Mathlib's
\texttt{Nat.choose\_succ\_right\_eq} and is applied once, at the end of the proof of
Theorem~\ref{thm:lower_msets}.
...
\textbf{Lean remark.} Lemmas~\ref{lem:msets_good_set} and~\ref{lem:msets_pinsker_step} are
carried out inside the proof of Theorem~\ref{thm:lower_msets} rather than as separate
declarations, since they speak about the run measures of the fixed algorithm: $G_B$ is a
\texttt{Finset.filter} of $B^c$, the counting argument bounds the cardinalities of the two
exceptional sets by $n/4$ each from $\sum_{i}\E N_i = mT$ and $\sum_i\Prob(A_i) = m$, and the
Pinsker step is Lemma~\ref{lem:two_point_pinsker} in its data-dependent form.

\begin{theorem}[Lower bound for $m$-sets]\label{thm:lower_msets}
\lean{Maiti2026Power.le_of_isPAC_mSet}\leanok
...
\begin{corollary}[PAC lower bound for $m$-sets]\label{cor:lower_msets_pac}
\lean{Maiti2026Power.mSet_lt_budget_of_isPAC}\leanok
\uses{thm:lower_msets, def:pac}
Let $\Xs$ be the $m$-sets with $n = d - m + 1 \ge 20m$ and $\eps > 0$. An $(\eps,\delta)$-PAC
algorithm on $\Xs$ with budget $T \ge 1$ and $\delta < 2/9$ has $T > mn/(2500\,\eps^2)$.
\end{corollary}
\begin{proof}
\leanok
\uses{thm:lower_msets}
As for Corollary~\ref{cor:lower_multitask_pac}, from Theorem~\ref{thm:lower_msets}.
\end{proof}
...
\textbf{Lean remark.} The Lean statement \texttt{le\_of\_isPAC\_mSet} is the contrapositive
form: an $(\eps,\delta)$-PAC algorithm with $1 \le T \le mn/(2500\eps^2)$ has $\delta \ge 2/9$.
It compares the upper bound $7\eps\binom dm$ on $\sum_{\abs S = m}\E_{\theta^{(S)}}\ip{\rec}{\theta^{(S)}}$
with the lower bound $9\eps(1-\delta)\binom dm$ coming from
Lemma~\ref{lem:fail_prob_from_expected_value} (PAC form) and
$Z(\theta^{(S)}) = 10\eps$. The sizes are handled by $m = k+1$ and $d = k+n$ rather than by
...
\lean{Maiti2026Power.unitBall_le_budget_of_isPAC}\leanok
