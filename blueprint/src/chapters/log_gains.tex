...
\uses{thm:lower_hypercube_pm, def:pac}
Let $\eps > 0$. An $(\eps,\delta)$-PAC algorithm on $\{-1,1\}^d$ with budget $T \ge 1$ and
$\delta < 1/6$ has $T > d^2/(100\,\eps^2)$.
\end{corollary}
\begin{proof}
\uses{thm:lower_hypercube_pm}
If $T \le d^2/(100\eps^2)$, Theorem~\ref{thm:lower_hypercube_pm} gives $s$ with
$\Prob_{\theta_s}(r(\rec,\theta_s) > \eps) \ge 1/6 > \delta$, contradicting the PAC property.
\end{proof}
...
\uses{thm:lower_hypercube_01, def:pac}
Let $\eps > 0$. An $(\eps,\delta)$-PAC algorithm on $\{0,1\}^d$ with budget $T \ge 1$ and
$\delta < 1/6$ has $T > d^2/(400\,\eps^2)$.
\end{corollary}
\begin{proof}
\uses{thm:lower_hypercube_01}
As for Corollary~\ref{cor:lower_hypercube_pm_pac}, from Theorem~\ref{thm:lower_hypercube_01}.
\end{proof}
