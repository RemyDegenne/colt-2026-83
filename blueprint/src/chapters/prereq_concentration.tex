...
\lean{ProbabilityTheory.gaussianConcentrationConst}\leanok
$\cG := \pi^2/4$ ($\approx 2.4674$).
\end{definition}

\begin{lemma}[MGF of a linear form of a standard Gaussian vector]\label{lem:gauss_mgf_inner}
\lean{ProbabilityTheory.stdGaussian_map_toDual, ProbabilityTheory.integrable_exp_mul_inner_stdGaussian, ProbabilityTheory.integral_exp_mul_inner_stdGaussian, ProbabilityTheory.integral_exp_inner_stdGaussian}\leanok
...
\lean{ProbabilityTheory.IsGaussian.integrable_exp_mul_norm, ProbabilityTheory.IsGaussian.integrable_exp_of_abs_le_add_mul_norm, ProbabilityTheory.IsGaussian.integrable_of_abs_le_add_mul_norm}\leanok
\uses{def:pg_gaussian_vector}
...
\lean{ProbabilityTheory.lipschitzWith_of_norm_gradient_le, ProbabilityTheory.abs_le_of_norm_gradient_le, ProbabilityTheory.abs_inner_gradient_le, ProbabilityTheory.ContDiff.continuous_gradient, ProbabilityTheory.IsGaussian.integrable_exp_of_norm_gradient_le, ProbabilityTheory.IsGaussian.integrable_of_norm_gradient_le}\leanok
...
\lean{softmax, logSumExp, sum_exp_inner_pos, softmax_nonneg, sum_softmax, contDiff_logSumExp, hasGradientAt_logSumExp, norm_gradient_logSumExp_le, ciSup_inner_le_logSumExp, logSumExp_le_ciSup_inner_add}\leanok
...
\lean{ProbabilityTheory.hasDerivAt_comp_quarterCircle, ProbabilityTheory.sub_eq_integral_quarterCircle}\leanok
...
\lean{ProbabilityTheory.exp_mul_integral_le_integral_exp}\leanok
...
\lean{ProbabilityTheory.integral_comp_rotation_swap_prod, ProbabilityTheory.integral_exp_inner_gradient_prod_stdGaussian_le}\leanok
...
\lean{ProbabilityTheory.hasSubgaussianMGF_sub_integral_stdGaussian}\leanok
...
\lean{ProbabilityTheory.abs_ciSup_inner_le, ProbabilityTheory.measurable_ciSup_inner, ProbabilityTheory.hasSubgaussianMGF_iSup_inner_stdGaussian}\leanok
\uses{def:pg_gaussian_vector, def:pc_gaussian_constant}
...
\lean{ProbabilityTheory.hasSubgaussianMGF_supportFn_stdGaussian}\leanok
...
\textbf{Lean remark.} The formalization states Lemmas~\ref{lem:finite_sup_subgaussian},
\ref{lem:compact_sup_subgaussian} and Theorem~\ref{thm:borell_tis} for the standard Gaussian
measure $\Normal(0, I_d)$ (Mathlib \texttt{stdGaussian E}, on any finite-dimensional inner
product space $E$) and a bound $\sigma$ on the norms of the index vectors, which is the case
$S = I_d$; the case of a general covariance $S \in \PSD$ is the image of this one under
$S^{1/2}$ (Lemma~\ref{lem:gauss_law_of_cov}(iii) and \texttt{supportFn\_image}), see
\texttt{ProbabilityTheory.hasSubgaussianMGF\_supportFn\_multivariateGaussian}. The supremum
over $\Xs$ is \texttt{supportFn K} and the partial maxima are \texttt{partialSupInner q n}
(\texttt{⨆ i : Fin (n+1), ⟪q i, v⟫}) for the dense sequence of
Lemma~\ref{lem:sup_countable_dense}; the two limits are \texttt{Filter.Tendsto} statements
along \texttt{atTop} proved by dominated convergence. Lemma~\ref{lem:finite_sup_subgaussian}
is stated for a family \texttt{y : ι → E} indexed by a nonempty finite type, and the limit
$\beta \to \infty$ is \texttt{ge\_of\_tendsto}.

\begin{theorem}[Borell--TIS inequality for linear Gaussian processes]\label{thm:borell_tis}
\lean{ProbabilityTheory.measureReal_supportFn_sub_integral_ge_le, ProbabilityTheory.measureReal_supportFn_sub_integral_le_le, ProbabilityTheory.hasSubgaussianMGF_supportFn_multivariateGaussian}\leanok
\uses{def:pg_gaussian_vector, def:pc_gaussian_constant, lem:sup_countable_dense}
...
\lean{COLT83.integral_diffSup_multivariateGaussian, COLT83.measureReal_diffSup_gt_le}\leanok
