...
Mathlib also lacks the behaviour of $\KL$ under restrictions (Lemma~\ref{lem:kl_restrict}), the
invariance under sufficient statistics (Lemma~\ref{lem:kl_sufficient_statistic}) and the
continuity of $\KL$ along a generating sequence of $\sigma$-algebras
(Lemma~\ref{lem:kl_iSup_map}), which the stopping-time and trajectory versions of the
decomposition (Section~\ref{sec:pre_info_stopping}) rely on. All of these now live in LML
(\texttt{LeanMachineLearning/ForMathlib/InformationTheory/KullbackLeibler/\{ChainRule, CompProd,
Convex, DataProcessing, MapSequence, Restrict\}.lean}), together with the policy/reward
specialization of Lemma~\ref{lem:pi_kl_step} and the divergence decomposition for
algorithm-environment sequences, for a fixed number of rounds and for whole trajectories
(\texttt{LeanMachineLearning/SequentialLearning/DivergenceDecomposition.lean}). What is left in
this repository is the divergence of two Gaussians, the two inequalities, and the versions of
the decomposition for stopped histories (Section~\ref{sec:pre_info_stopping}).
...
\lean{InformationTheory.klBerReal, InformationTheory.klBer, InformationTheory.klDiv_bernoulliMeasure_eq_klBerReal, InformationTheory.klDiv_bernoulliMeasure, InformationTheory.ofReal_le_klDiv_bernoulliMeasure, ProbabilityTheory.lintegral_bernoulliMeasure, ProbabilityTheory.bernoulliMeasure_eq_withDensity, ProbabilityTheory.rnDeriv_bernoulliMeasure}\leanok
...
\lean{InformationTheory.sq_sub_le_klBerReal, InformationTheory.sq_sub_le_klBer}\leanok
...
\lean{InformationTheory.sq_sub_le_klDiv, InformationTheory.abs_sub_le_sqrt_klDiv}\leanok
...
\lean{ProbabilityTheory.Kernel.compProd_prodMkLeft_apply, Learning.klDiv_compProd_compProd_prodMkLeft_eq_klDiv_comp_compProd, Learning.klDiv_compProd_compProd_prodMkLeft}\leanok
...
\lean{Learning.IdentAlg.IsFixedBudget.stoppingTime_eq, Learning.IdentAlg.IsFixedBudget.stoppedHist_eq, Learning.IdentAlg.IsFixedBudget.stoppingTime_ne_top, Learning.IdentAlg.IsRun.klDiv_map_stoppedHist_out, Learning.IdentAlg.IsRun.klDiv_map_out_le, Learning.LinearBandit.IsRun.klDiv_map_out_le, Learning.LinearBandit.IsRun.klDiv_map_out_le_of_sq_le, Learning.LinearBandit.IsRun.exp_neg_le_measureReal_add_of_sq_le}\leanok
...
(history, output) is the composition-product with the output kernel, a Markov kernel, so
\texttt{klDiv\_compProd\_left} applies (\texttt{IsRun.klDiv\_map\_stoppedHist\_out}). Only the
inequality
...
StoppedHistory, DivergenceDecomposition, RunDivergence\}.lean}, on top of LML's stopping rules
and stopped histories (\texttt{LeanMachineLearning/SequentialLearning/StoppedHistory.lean}) and
of the $\KL$ lemmas of
\texttt{LeanMachineLearning/ForMathlib/InformationTheory/KullbackLeibler/\{CompProd,
DataProcessing, MapSequence, Restrict\}.lean} and of the fixed-round and trajectory
decompositions of \texttt{LeanMachineLearning/SequentialLearning/DivergenceDecomposition.lean}.
...
\lean{Learning.sigmaHistory, Learning.lt_hittingAfter_sigmaHistory_iff, Learning.hittingAfter_sigmaHistory_eq_coe_iff, Learning.stoppedValue_sigmaHistory_of_eq, Learning.fst_stoppedProcess_sigmaHistory_le, Learning.measurable_hittingAfter_sigmaHistory, Learning.measurable_stoppedValue_sigmaHistory, Learning.measurable_stoppedProcess_sigmaHistory, Learning.IsAlgEnvSeq.adapted_sigmaHistory, Learning.exists_measurableSet_preimage_lt_hittingAfter_sigmaHistory, Learning.truncHist, Learning.truncHist_stoppedValue_sigmaHistory, Learning.stoppedProcess_sigmaHistory_ae_eq_truncHist, ProbabilityTheory.HasLaw.stoppedProcess_sigmaHistory, Learning.restrict_map_truncHist, Learning.stoppedHistMeasure, Learning.IsAlgEnvSeq.hasLaw_stoppedValue_sigmaHistory, Learning.IdentAlg.stoppingTime, Learning.IdentAlg.stoppedHist, Learning.IdentAlg.isStoppingTime_stoppingTime}\leanok
...
$\tau_S := \inf\{n : \mathrm{hist}_n \in S\} \in \N\cup\{\infty\}$ is the number of rounds
played: the hitting time (Mathlib \texttt{hittingAfter}) of $S$ by the process
$n \mapsto \langle n, \mathrm{hist}_n\rangle$ of histories of variable length (LML
\texttt{sigmaHistory}); it is a stopping time of the history filtration of any
...
rounds as a history of variable length (Mathlib \texttt{stoppedValue}; of arbitrary length if
$\tau = \infty$); it belongs to $S$ when $\tau = \tau_S < \infty$. The history stopped at time
$M$, $\mathrm{hist}_{\min(\tau,M)}$, is Mathlib's \texttt{stoppedProcess}. The \emph{truncation}
$\mathrm{trunc}_M$ keeps the first $M$ rounds of a history of variable length:
$\mathrm{trunc}_M(\mathrm{hist}_\tau) = \mathrm{hist}_{\min(\tau,M)}$ when $\tau < \infty$, so
that the law of $\mathrm{hist}_{\min(\tau,M)}$ is the image of the law of $\mathrm{hist}_\tau$ by
$\mathrm{trunc}_M$ when $\tau$ is almost surely finite, and both laws agree on histories of
length $< M$. The law of $\mathrm{hist}_{\tau_S}$ is determined by the algorithm and the
environment (LML \texttt{stoppedHistMeasure}). The stopping time and stopped history of an
identification algorithm (Definition~\ref{def:bai_alg}) are those of its stopping rule.
\end{definition}
...
\lean{Learning.map_stoppedProcess_sigmaHistory_eq_add, Learning.map_stoppedProcess_sigmaHistory_succ_eq_add, Learning.hasLaw_stoppedProcess_sigmaHistory_zero, Learning.IsAlgEnvSeq.hasCondDistrib_step_restrict_lt_hittingAfter_sigmaHistory, Learning.IsAlgEnvSeq.hasCondDistrib_obs_restrict_lt_hittingAfter_sigmaHistory, Learning.IsAlgEnvSeq.hasCondDistrib_action_restrict_lt_hittingAfter_sigmaHistory, Learning.IsAlgEnvSeq.klDiv_map_stoppedProcess_sigmaHistory_stepKernel, Learning.IsAlgEnvSeq.klDiv_map_stoppedProcess_sigmaHistory_compProd, Learning.IsAlgEnvSeq.klDiv_map_stoppedProcess_sigmaHistory, Learning.sum_ite_lt_eq_sum_range_toNat_min}\leanok
...
\lean{Learning.IsAlgEnvSeq.klDiv_map_stoppedValue_sigmaHistory_stepKernel, Learning.IsAlgEnvSeq.klDiv_map_stoppedValue_sigmaHistory_compProd, Learning.IsAlgEnvSeq.klDiv_map_stoppedValue_sigmaHistory, Learning.tsum_ite_lt_eq_sum_range_toNat}\leanok
...
\lean{Learning.IdentAlg.IsRun.klDiv_map_stoppedHist_out, Learning.IdentAlg.IsRun.klDiv_map_out_le_stoppedHist, Learning.IdentAlg.IsRun.klDiv_map_out_le_tsum_compProd, Learning.IdentAlg.IsRun.klDiv_map_out_le_lintegral}\leanok
...
probability spaces $(\Omega,\Prob)$ and $(\Omega',\Prob')$. Then the divergence between the laws
of (stopped history, output) is the divergence between the laws of the stopped histories, and
the divergence between the laws of the outputs is at most the latter. If moreover the stopping
times are almost surely finite, then for two stationary environments with reward kernels
$\kappa,\kappa'$,
\[
...
As in Lemma~\ref{lem:kl_data_processing_recommendation}: the output is drawn from the same
Markov kernel applied to the stopped history under both laws, so \texttt{klDiv\_compProd\_left}
applies; then data processing along the projection on the output, and
Theorem~\ref{thm:divergence_decomposition_stopping}.
\end{proof}
