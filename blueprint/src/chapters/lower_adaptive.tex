...
since $M$ is symmetric (\texttt{Maiti2026Power.inner\_toEuclideanCLM\_normalizeMat}); the reward vectors
...
\lean{Maiti2026Power.normalizeMat, Maiti2026Power.IsGOptimalDesign.norm_normalizeMat_le, Maiti2026Power.IsGOptimalDesign.norm_normalizeMat_eq_one, Maiti2026Power.IsGOptimalDesign.designMatrix_mapDomain_normalizeMat, Maiti2026Power.transpose_normalizeMat, Maiti2026Power.inner_toEuclideanCLM_normalizeMat}\leanok
...
\lean{Maiti2026Power.IsGOptimalDesign.sum_mul_inner_normalizeMat_sq, Maiti2026Power.IsGOptimalDesign.exists_inner_normalizeMat_le, Maiti2026Power.IsGOptimalDesign.norm_sub_sq_normalizeMat, Maiti2026Power.IsGOptimalDesign.exists_separated_pair}\leanok
...
the proof of Lemma~\ref{lem:baseline_lower} (\texttt{Maiti2026Power.baseline\_le\_budget\_of\_isPAC}),
...
(\texttt{IdentAlg.isRun\_runMeasure}).
...
\lean{Maiti2026Power.baseline_le_budget_of_isPAC}\leanok
...
\lean{Maiti2026Power.mixtureParam, Maiti2026Power.designWeight, Maiti2026Power.histLaw, Maiti2026Power.mixtureHistLaw, Maiti2026Power.IsDesign.isProbabilityMeasure_mixtureHistLaw, Maiti2026Power.IsGOptimalDesign.inner_mixtureParam_self}\leanok
...
(\texttt{Maiti2026Power.mixtureParam}: $\theta^{(j)} = 3\eps M(Mx_j)$, so that
$\ip{x}{\theta^{(j)}} = 3\eps\ip{Mx}{v_j}$), the laws $\Prob^{\mathrm{test}}_\theta$ are the laws
\texttt{Maiti2026Power.histLaw} of the history of length $N$ under the canonical trajectory measure of
...
\texttt{Maiti2026Power.mixtureHistLaw}.

\begin{lemma}[KL divergence to the mixture]\label{lem:mixture_kl}
\lean{Maiti2026Power.IsGOptimalDesign.sum_mul_inner_mixtureParam_sq, Maiti2026Power.IsGOptimalDesign.sum_mul_inner_mixtureParam_sq_le, Maiti2026Power.IsGOptimalDesign.klDiv_histLaw_zero_mixtureHistLaw_le}\leanok
...
\lean{Maiti2026Power.IsGOptimalDesign.exp_neg_le_of_mixture}\leanok
\uses{def:mixture_test}
...
(\texttt{Maiti2026Power.IsGOptimalDesign.exp\_neg\_le\_of\_mixture}); the passage to the bound on $N$
...
\texttt{thenRepeat alg n ρ} plays $\mathrm{alg}$ for the first $n+1$ rounds):
its policy is that of $\mathrm{alg}$ at the rounds $t < n+1$, the kernel $\rho$ applied to the
history of the first $n+1$ rounds at round $t = n+1$, and the deterministic policy ``play the
action recorded at index $n+1$ of the history'' afterwards; the recommendation of $\mathsf{Alg}$
is thus the action $x_{n+1}$. The three facts used about a run of this algorithm are: it is an
algorithm-environment sequence for $\mathrm{alg}$ until time $n+1$
(\texttt{IsAlgEnvSeq.isAlgEnvSeqUntil\_of\_thenRepeat}), $x_{n+1}$ has conditional law
$\rho(\hist_{n+1})$ given $\hist_{n+1}$ (\texttt{hasCondDistrib\_action\_succ\_of\_thenRepeat}),
and $x_{n+1+s} = x_{n+1}$ almost surely (\texttt{action\_add\_ae\_eq\_of\_thenRepeat}). The PAC
guarantee of $\mathsf{Alg}$ is transferred to $x_{n+1}$ by Lemma~\ref{lem:prefix_transfer}
(\texttt{IsPAC.measureReal\_bad\_action\_succ\_of\_testAlg}). The concentration of
...
\lean{Maiti2026Power.le_budget_of_isPAC}\leanok
