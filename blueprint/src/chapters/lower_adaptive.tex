...
(Lemma~\ref{lem:gauss_mean_concentration}) and the sequential composition of algorithms
...
\textbf{Lean remark.} The instance transformation acts on the \emph{action type}: the
algorithms of $\mathsf{Alg}$ have actions in the subtype $\Xs$ and those of $\mathsf{Alg}'$ in
the subtype $M\Xs$; $\phi$ is a \texttt{MeasurableEquiv} (indeed a \texttt{Homeomorph})
between the two subtypes, and $\mathrm{alg}'$ is obtained by transporting the initial
distribution and the policy kernels of $\mathrm{alg}$ along $\phi$ and $\Phi_t$
(\texttt{Kernel.map} and \texttt{Kernel.comap}). The identification of the law is a general
statement about LML algorithm-environment sequences transported along a measurable
equivalence of the action type; it should be proved once in that generality (the
environment $\nu'_{\theta'}$ composed with $\phi$ is $\nu_\theta$).
...
\uses{def:mixture_test, def:pac, lem:composition}
...
\begin{proof}
\uses{lem:composition, lem:noise_representation, lem:gauss_mean_concentration, def:normalized_instance, def:pac, def:bai_alg}
...
\textbf{Lean remark.} The test is built by Lemma~\ref{lem:composition} and not by adding
$\rec$ to the state: the second-phase algorithm $\mathrm{alg}_2(h)$ is the LML algorithm with
initial distribution $\rho(h)$ and the deterministic policy ``play the action recorded at
index $n$ of the history''; the recommendation of $\mathsf{Alg}$ is thus the action $x_n$,
and the identity between the law of $(\hist_n, x_n)$ and $\Prob_\theta$ of
Definition~\ref{def:bai_alg} is the composition-product structure of
Lemma~\ref{lem:composition}; the good event is the $h$-independent family
$E_h = \{\abs{\bar\eta} < \eps/2\}$, fed to Lemma~\ref{lem:composition} with $\delta' = 0$, so
no conditional probability given $\hist_n$ has to be manipulated. The estimate $\hat v$ is the empirical mean of the observations
of the second phase (LML \texttt{empMean}-style sums over the index range $[n, N)$), and
$\kappa$ is \texttt{Kernel.deterministic} of the map $\hist_N \mapsto \indic\{\hat v > \eps\}$.
...
satisfies
\[
  n \;\ge\; c_{\mathrm{ad}}\,\frac{d\log(1/\delta)}{\eps^2},
  \qquad
  c_{\mathrm{ad}} := \frac{1}{11556 + 5760\sqrt2} \;\ge\; \frac{1}{20000} .
\]
\end{theorem}
