...
\lean{Maiti2026Power.mixDesign, Maiti2026Power.designMatrix_mixDesign, Maiti2026Power.IsDesign.mixDesign, Maiti2026Power.posDef_designMatrix_mixDesign}\leanok
...
\lean{Maiti2026Power.smul_designMatrix_le_designMatrix_mixDesign_left, Maiti2026Power.smul_designMatrix_le_designMatrix_mixDesign_right, Maiti2026Power.IsGOptimalDesign.dotProduct_inv_designMatrix_mixDesign_le, Maiti2026Power.gwMat_designMatrix_mixDesign_le}\leanok
...
\textbf{Lean remark.} The mixture \texttt{Maiti2026Power.mixDesign w₁ w₂} and the two bounds are
formalized for an arbitrary design distribution $\lambda_1$ with $A(\lambda_1) \succ 0$ (the
width bound reads $\gwidth{\Xs}{A_0} \le \sqrt2\,\gwidth{\Xs}{A(\lambda_1)}$); the choice of a
width-minimizing $\lambda_1$ (Lemma~\ref{lem:width_attained}) is made only when the algorithm
is assembled.
...
\lean{Maiti2026Power.leastSquares, Maiti2026Power.designRows, Maiti2026Power.lsMatrix, Maiti2026Power.leastSquares_eq_toEuclideanLin}\leanok
...
\lean{Maiti2026Power.leastSquares_inner_add, Maiti2026Power.lsMatrix_mul_transpose, Maiti2026Power.hasLaw_leastSquares_of_fixedDesign}\leanok
...
\textbf{Lean remark.} The formalized statement is
$\hat\theta \sim \Normal(\theta, \Sigma_T^{-1})$ (\texttt{Maiti2026Power.hasLaw\_leastSquares\_of\_fixedDesign},
for the run of the fixed design in the environment $\nu_\theta$ and any $T$ with
$\Sigma_T \succ 0$), obtained from $\hat\theta = \theta + \Sigma_T^{-1}X^\top\eta$
(\texttt{leastSquares\_inner\_add}), the standard Gaussian law of $\eta$
(Lemma~\ref{lem:fixed_design_law}) and \texttt{multivariateGaussian\_zero\_map\_toEuclideanLin}
with $M M^\top = \Sigma_T^{-1}$ (\texttt{lsMatrix\_mul\_transpose}); the representation
$\Sigma_T^{-1/2}g$ is not needed.

\begin{lemma}[Measurable approximate argmax selector]\label{lem:approx_argmax_selector}
\lean{exists_measurable_of_forall_exists_mem, exists_measurable_approx_argmax, exists_measurable_argmax_of_finite}\leanok
...
\lean{Maiti2026Power.simpleRegret_le_supportFn_add_supportFn}\leanok
\uses{def:regret}
...
\textbf{Lean remark.} The formalized bound is
$r(\rec,\theta) \le \sup_{x\in\Xs}\ip{x}{\theta - \hat\theta} + \sup_{x\in\Xs}\ip{x}{\hat\theta-\theta} + \gamma$
(the two support functions add up to $D$), for a bounded set $\Xs$ and without choosing a
maximizer $x^\star$: it follows from the subadditivity of the support function
$\sup_x\ip{x}{\theta} \le \sup_x\ip{x}{\theta-\hat\theta} + \sup_x\ip{x}{\hat\theta}$.

\begin{lemma}[Concentration of the difference process]\label{lem:difference_process_concentration}
\lean{Maiti2026Power.diffSup, Maiti2026Power.diffSup_eq_supportFn_add_neg, Maiti2026Power.continuous_diffSup, Maiti2026Power.integral_diffSup_multivariateGaussian, Maiti2026Power.measureReal_diffSup_gt_le}\leanok
...
\lean{Maiti2026Power.lsRecommend, Maiti2026Power.measurable_lsRecommend, Maiti2026Power.fixedDesignIdentAlg, Maiti2026Power.isFixedBudget_fixedDesignIdentAlg, Maiti2026Power.isFixedDesign_fixedDesignIdentAlg, Maiti2026Power.output_fixedDesignIdentAlg}\leanok
...
\lean{Maiti2026Power.isPAC_fixedDesignIdentAlg}\leanok
...
\lean{Maiti2026Power.exists_isFixedDesign_isPAC}\leanok
...
\textbf{Lean remark.} The formalized algorithm \texttt{Maiti2026Power.fixedDesignIdentAlg T x s}
takes the design $x$ and the selector $s$ as parameters, and
\texttt{Maiti2026Power.isPAC\_fixedDesignIdentAlg} is the guarantee in the general form ``if
$\Sigma_T \succ 0$, $\norm{x}^2_{\Sigma_T^{-1}} \le \sigma^2$ on $\Xs$, $s$ is an $\eps/4$-selector
and $2\gwidth{\Xs}{\Sigma_T} + 2\sigma\sqrt{2\cG\log(1/\delta)} \le 3\eps/4$, then the
algorithm is $(\eps,\delta)$-PAC''; the choice of the design and the arithmetic of the
constants are done in the proof of Corollary~\ref{cor:upper_exists}
(\texttt{Maiti2026Power.exists\_isFixedDesign\_isPAC}). Instead of a width-minimizing design
$\lambda_1$ (Lemma~\ref{lem:width_attained}, not formalized) the formal proof uses an
approximate minimizer with $\gwidth{\Xs}{A_1}^2 \le \gw(\Xs)^2 + (\log 2)/4$, which the slack
$337.5 - 320 = 17.5$ of the arithmetic absorbs. The case $d = 0$ is treated separately (every
recommendation has zero regret). The theorem is an existence statement whose witness is built from
...
\lean{Maiti2026Power.lnNoise_real_norm_lnDelta_gt_le, Maiti2026Power.norm_sub_inner_normalizeBall_le}
...
\textbf{Lean remark.} The two steps of the proof are formalized on the noise space
$(\R^n)^d$ with the law $\Normal(0,1)^{\otimes dn}$ of the noises of the basis rounds: the
chi-square tail bound $\Prob(\norm{\Delta} > c) \le \delta$ whenever
$2d + 12\log(1/\delta) \le nc^2$ (\texttt{lnNoise\_real\_norm\_lnDelta\_gt\_le}, with
$\Delta$ the vector of noise averages) and the deterministic regret bound
$\norm{\vartheta} - \ip{v/\norm v}{\vartheta} \le 2\norm{v - \vartheta}$
(\texttt{norm\_sub\_inner\_normalizeBall\_le}, with $v/\norm v := 0$ when $v = 0$). The
fixed-design algorithm itself is not formalized on its own: it is the second phase of the
block algorithm of Definition~\ref{def:block_algorithm}, where the above bounds are used
with $(\eps/2, \delta/2)$.
