...
\lean{Matrix.dotProduct_mulVec_le_of_le, Matrix.le_iff_dotProduct_mulVec_le}\leanok
...
\lean{Matrix.mul_mul_conjTranspose_le_of_le, Matrix.conjTranspose_mul_mul_le_of_le, Matrix.PosDef.mul_mul_conjTranspose_same}\leanok
...
\lean{Matrix.IsHermitian.trace_cfc, Matrix.IsHermitian.trace_inv_sub_smul_one, Matrix.IsHermitian.trace_inv_sub_smul_one_sq, Matrix.IsHermitian.posSemidef_sub_smul_one_iff, Matrix.IsHermitian.posDef_sub_smul_one_iff, Matrix.IsHermitian.smul_one_le_iff, Matrix.PosDef.pos_of_mem_spectrum}
\uses{lem:loewner_quadratic}
...
The formalization never sorts the eigenvalues: the condition ``$\lambda_{\min}(A) > c$'' is
encoded as $\forall x \in \operatorname{spec}_{\R}(A),\ c < x$ (the real spectrum is the range
of the eigenvalues, \texttt{Matrix.IsHermitian.spectrum\_real\_eq\_range\_eigenvalues}), which
is equivalent to $A - cI \in \PD$ (\texttt{Matrix.IsHermitian.posDef\_sub\_smul\_one\_iff}), and
the traces of functions of $A$ are computed through the continuous functional calculus:
$\tr(\texttt{cfc}\ f\ A) = \sum_j f(\lambda_j)$ (\texttt{Matrix.IsHermitian.trace\_cfc}), so that
$\tr((A - cI)^{-1}) = \sum_j (\lambda_j - c)^{-1}$ and $\tr((A-cI)^{-2}) = \sum_j(\lambda_j - c)^{-2}$
for $c$ outside the spectrum. This covers the parts of (i), (ii), (iv), (v) used by the rounding
argument; the operator-norm statements (iii) and the bound $\tr(A^2) \le d\lambda_{\max}^2$ are not
formalized.

\begin{lemma}[Square roots of positive definite matrices]\label{lem:sqrt_props}
\lean{Matrix.PosDef.sqrt, Matrix.PosSemidef.sqrt_mul_sqrt, Matrix.PosDef.sqrt_mul_sqrt_inv, Matrix.PosDef.sqrt_inv_mul_sqrt, Matrix.PosDef.sqrt_inv_mul_mul_sqrt_inv, Matrix.PosDef.sqrt_mul_inv_mul_sqrt, Matrix.transpose_sqrt, Matrix.norm_toEuclideanCLM_sqrt_sq, Matrix.norm_toEuclideanCLM_sqrt_inv_sq, Matrix.inner_toEuclideanCLM_sqrt_sqrt_inv}
...
The formalization uses \texttt{CFC.sqrt} and proves the items (i), (iii), (iv), (v) directly from
\texttt{CFC.sqrt\_mul\_sqrt\_self} and \texttt{Matrix.PosSemidef.inv\_sqrt}, without the spectral
formula; the eigenvalue statements of (i)--(ii) and the scaling rule (vi) are not formalized yet
(the latter is proved inline where it is used, in \texttt{Maiti2026Power.gwMat\_smul}).

\begin{lemma}[Inversion reverses the Loewner order]\label{lem:loewner_inv}
\lean{Matrix.PosDef.of_le, Matrix.PosDef.inv_le_inv}\leanok
...
\lean{Matrix.PosDef.inv_le_inv_smul, Matrix.PosDef.dotProduct_inv_mulVec_le_of_smul_le}\leanok
...
\lean{Matrix.PosDef.trace_inv_eq_sum_inv_eigenvalues, Matrix.PosDef.card_sq_le_trace_mul_trace_inv, Matrix.PosDef.card_sq_div_trace_le_trace_inv}\leanok
\uses{lem:eigen_quadratic_bounds}
...
\lean{Maiti2026Power.gValue}\leanok
...
\lean{Maiti2026Power.sum_mul_dotProduct_inv_mulVec, Maiti2026Power.IsDesign.card_le_gValue, Maiti2026Power.IsDesign.dotProduct_inv_mulVec_eq_card}\leanok
...
\lean{Maiti2026Power.exists_isMaxOn_det_designSet, Maiti2026Power.posDef_of_isMaxOn_det}\leanok
...
\begin{lemma}[Determinant along a rank-one mixture]\label{lem:det_rank_one_mix}
\lean{Matrix.det_add_vecMulVec, Maiti2026Power.det_smul_add_smul_outerSelf}\leanok
Let $A \in \R^{d\times d}$ be invertible and $u, v \in \R^d$. Then (matrix determinant lemma)
\[
  \det(A + uv^\top) = \det A\,\big(1 + v^\top A^{-1} u\big) .
\]
Consequently, for every $x \in \R^d$ and $t \ne 1$,
\[
  \det\big((1-t)A + t\,xx^\top\big) = (1-t)^d\,\det A\,\Big(1 + \frac{t}{1-t}\,x^\top A^{-1}x\Big) .
\]
\end{lemma}

\begin{proof}
\leanok
$A + uv^\top = A\,(I + (A^{-1}u)v^\top)$ and $\det(I + wv^\top) = 1 + v^\top w$ (the
Weinstein--Aronszajn identity $\det(I + CD) = \det(I + DC)$ with $C = w$ a column and $D = v^\top$
a row; Mathlib's \texttt{Matrix.det\_one\_add\_replicateCol\_mul\_replicateRow}). For the second
identity apply the first to the invertible matrix $(1-t)A$, whose determinant is $(1-t)^d\det A$
and whose inverse is $(1-t)^{-1}A^{-1}$, with $u = t\,x$ and $v = x$.
\end{proof}

\begin{lemma}[Sign of the derivative at a one-sided maximum]\label{lem:deriv_nonpos_right_max}
\lean{HasDerivAt.nonpos_of_eventually_le_nhdsGT}\leanok
Let $f : \R \to \R$ be differentiable at $a$ and assume that $f(t) \le f(a)$ for all $t$ in a
right neighborhood of $a$. Then $f'(a) \le 0$.
\end{lemma}

\begin{proof}
\leanok
The difference quotients $(f(a+t) - f(a))/t$ are $\le 0$ for small $t > 0$ and converge to
$f'(a)$ as $t \to 0^+$ (\texttt{HasDerivAt.tendsto\_slope\_zero\_right}).
\end{proof}

\textbf{Lean remark.} The outline proves the Kiefer--Wolfowitz theorem through the directional
derivative $\frac{d}{dt}\log\det(A + tM)|_{t=0} = \tr(A^{-1}M)$ for a symmetric direction $M$,
whose proof goes through the spectral theorem. Since $D$-optimality is only tested along the
segments from $A_D$ to the rank-one matrices $xx^\top$, the derivative of
$t \mapsto \det((1-t)A_D + t\,xx^\top)$ at $t = 0$ is available in closed form from
Lemma~\ref{lem:det_rank_one_mix}, and the formalization follows this elementary route.

\begin{theorem}[Kiefer--Wolfowitz: existence of a $G$-optimal design]\label{thm:kiefer_wolfowitz}
\lean{Maiti2026Power.IsGOptimalDesign, Maiti2026Power.exists_isGOptimalDesign, Maiti2026Power.IsGOptimalDesign.gValue_eq, Maiti2026Power.IsGOptimalDesign.dotProduct_inv_mulVec_eq_card}\leanok
...
\begin{proof}
\leanok
\uses{lem:exists_d_optimal, def:design_matrix, lem:caratheodory_design, lem:designset_basic, lem:det_rank_one_mix, lem:deriv_nonpos_right_max, lem:g_ge_d}
...
with $\abs{\supp\lambda_G} \le d^2+1$, which we do not need); write $A_G := A_D$.

Fix $x \in \Xs$ and set $g := x^\top A_G^{-1}x$. For $t \in [0,1]$, $B := xx^\top = A(\delta_x)
\in \designset(\Xs)$ and $(1-t)A_G + tB \in \designset(\Xs)$ by convexity
(Lemma~\ref{lem:designset_basic}), so by $D$-optimality $\det((1-t)A_G + t\,xx^\top) \le \det A_G$.
By Lemma~\ref{lem:det_rank_one_mix} and $\det A_G > 0$, this says that
\[
  \varphi(t) := (1-t)^d\Big(1 + \frac{t}{1-t}\,g\Big) \le 1 = \varphi(0)
  \qquad\text{for all } t \in (0,1) .
\]
The function $\varphi$ is differentiable at $0$ with $\varphi'(0) = -d + g$ (product rule:
$\frac{d}{dt}(1-t)^d|_{0} = -d$ and $\frac{d}{dt}\frac{t}{1-t}|_0 = 1$), so
Lemma~\ref{lem:deriv_nonpos_right_max} gives $g - d \le 0$, i.e.\ $x^\top A_G^{-1}x \le d$.
...
\textbf{Lean remark.} The formalization packages the conclusion as the predicate
\texttt{Maiti2026Power.IsGOptimalDesign 𝒳 w}: $w$ is a design distribution on $\Xs$
(\texttt{Maiti2026Power.IsDesign}), its design matrix is positive definite and
$x^\top A(w)^{-1}x \le d$ for all $x \in \Xs$; \texttt{Maiti2026Power.exists\_isGOptimalDesign} is the
existence statement, and the value $g_{\Xs}(A_G) = d$ and the support property are the lemmas
\texttt{IsGOptimalDesign.gValue\_eq} and \texttt{IsGOptimalDesign.dotProduct\_inv\_mulVec\_eq\_card}.
The theorem needs $d \ge 1$ (the type \texttt{ι} nonempty), as does
Lemma~\ref{lem:exists_posdef_design}.
...
Lemma~\ref{lem:caratheodory_design}, hence with $\abs{\supp\lambda_G} \le d^2 + 1$; the
paper (Appendix~B.1) states the existence of a $G$-optimal design with
$\abs{\supp\lambda_*} \le d(d+1)/2$ (the sharper bound uses Carath\'eodory's theorem in the
space of symmetric matrices and that the optimal matrices lie on the boundary of the hull). This
numerical bound is deliberately kept out of the statements of
Theorem~\ref{thm:kiefer_wolfowitz} and Lemma~\ref{lem:width_attained}: only the finiteness of
the support is ever used. In
...
\lean{Maiti2026Power.IsGOptimalDesign.norm_sqrt_inv_le, Maiti2026Power.IsGOptimalDesign.norm_sqrt_inv_eq}\leanok
...
\lean{Maiti2026Power.normalizeMat, Maiti2026Power.IsGOptimalDesign.norm_normalizeMat_eq_one, Maiti2026Power.IsGOptimalDesign.designMatrix_mapDomain_normalizeMat, Maiti2026Power.IsGOptimalDesign.norm_normalizeMat_le, Maiti2026Power.IsGOptimalDesign.isDesign_mapDomain_normalizeMat, Maiti2026Power.IsGOptimalDesign.span_image_normalizeMat_eq_top, Maiti2026Power.isUnit_normalizeMat}\leanok
...
\lean{Maiti2026Power.exists_isDesign_of_mem_designSet, Maiti2026Power.mem_designSet_iff}\leanok
\uses{def:design_matrix}
Every $A \in \designset(\Xs)$ is of the form $A = A(\lambda)$ for a design distribution
$\lambda \in \simplex_{\Xs}$ with $\abs{\supp\lambda} \le d^2 + 1$.
\end{lemma}

\begin{proof}
\leanok
\uses{def:design_matrix}
$\designset(\Xs) = \conv S$ with $S := \{xx^\top : x \in \Xs\} \subset \R^{d\times d}$, a real
vector space of dimension $d^2$. By Carath\'eodory's theorem, every point of $\conv S$
is a convex combination of an affinely independent family of points of $S$, which has at most
$d^2 + 1$ elements: $A = \sum_{i=1}^k \alpha_i s_i$ with $k \le d^2+1$, $\alpha_i > 0$,
...
$\lambda \in \simplex_{\Xs}$, $A(\lambda) = A$ and $\abs{\supp\lambda} = k \le d^2 + 1$.
\end{proof}

\textbf{Lean remark.} \texttt{eq\_pos\_convex\_span\_of\_mem\_convexHull}
(\texttt{Mathlib/Analysis/Convex/Caratheodory.lean}) writes a point of the hull as a positive
combination of an affinely independent family, and
\texttt{AffineIndependent.card\_le\_finrank\_succ} bounds its cardinality by
$\texttt{finrank} + 1$. Applying it in the space of symmetric matrices would give the bound
$d(d+1)/2 + 1$ (its \texttt{finrank} is not in Mathlib and would have to be computed from the
basis of matrices $E_{ii}$ and $E_{ij} + E_{ji}$, $i < j$); the formalization applies it in the
full matrix space $\texttt{Matrix ι ι ℝ}$, of dimension $d^2$, which is sufficient for every use
in this blueprint (the support size of $\lambda_G$ and $\lambda_1$ only needs to be finite).
Design distributions are encoded as finitely supported functions
$\texttt{EuclideanSpace ℝ ι →₀ ℝ}$ (\texttt{Finsupp}) with the predicate \texttt{Maiti2026Power.IsDesign}
(support in $\Xs$, nonnegative, summing to one), and $A(\lambda)$ is the linear map
\texttt{Maiti2026Power.designMatrix}; the equality of $\designset(\Xs)$ with the set of design matrices
of design distributions is \texttt{Maiti2026Power.mem\_designSet\_iff}.
...
\textbf{Lean remark.} Not formalized: the fixed-design algorithm of Chapter~\ref{chap:upper} is
built in Lean from an \emph{approximate} minimizer of the width (any $A_1$ with
$\gwidth{\Xs}{A_1}^2 \le \gw(\Xs)^2 + (\log 2)/4$, which exists by definition of the infimum),
so the attainment is not needed for Theorem~\ref{thm:upper}.
...
\lean{Maiti2026Power.barrier, Maiti2026Power.barrier_eq_sum, Maiti2026Power.barrier_neg_card_zero}\leanok
\uses{lem:eigen_quadratic_bounds}
...
\lean{Matrix.inv_add_vecMulVec, Matrix.trace_inv_add_vecMulVec_self}\leanok
...
\lean{Maiti2026Power.barrierScore, Maiti2026Power.barrier_lt_barrier_add_half, Maiti2026Power.barrier_step}\leanok
...
\lean{Maiti2026Power.barrier_average}\leanok
...
\lean{Maiti2026Power.exists_rounding}\leanok
...
``for every $t$ there is a family \texttt{x : Fin t → E} of points of the support whose $B_t$
satisfies the invariant'' (\texttt{Maiti2026Power.exists\_rounding}); the greedy choice at each step is
the point provided by Lemma~\ref{lem:barrier_average} (\texttt{Maiti2026Power.barrier\_average}, stated
for a design distribution with design matrix $I$, here the whitened distribution
$\lambda \circ (A^{-1/2})^{-1}$ of \texttt{IsDesign.mapDomain}), and the family is extended with
\texttt{Fin.snoc}. The invariant ``$\lambda_{\min}(B_t) > l_t$'' is the spectral condition
$\forall \mu \in \operatorname{spec}_{\R}(B_t),\ l_t < \mu$ (see the Lean remark after
Lemma~\ref{lem:eigen_quadratic_bounds}), and the eigenvalue inequality at the heart of
Lemma~\ref{lem:barrier_average} is isolated as \texttt{barrier\_sum\_ineq}. The design matrix
$\Sigma_T$ is $\sum_{t<T}$ \texttt{outerSelf (x t)} for \texttt{x : Fin T → E}. Note that
...
\lean{Maiti2026Power.posDef_of_smul_le, Maiti2026Power.dotProduct_inv_mulVec_le_of_smul_le, Maiti2026Power.gwMat_le_of_smul_le}\leanok
