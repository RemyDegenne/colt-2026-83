...
(the latter is proved inline where it is used, in \texttt{COLT83.gwMat\_smul}).
...
\lean{COLT83.sum_mul_dotProduct_inv_mulVec, COLT83.IsDesign.card_le_gValue, COLT83.IsDesign.dotProduct_inv_mulVec_eq_card}\leanok
...
\lean{COLT83.exists_isMaxOn_det_designSet, COLT83.posDef_of_isMaxOn_det}\leanok
...
\lean{Matrix.det_add_vecMulVec, COLT83.det_smul_add_smul_outerSelf}\leanok
...
\lean{COLT83.IsGOptimalDesign, COLT83.exists_isGOptimalDesign, COLT83.IsGOptimalDesign.gValue_eq, COLT83.IsGOptimalDesign.dotProduct_inv_mulVec_eq_card}\leanok
...
\texttt{COLT83.IsGOptimalDesign 𝒳 w}: $w$ is a design distribution on $\Xs$
(\texttt{COLT83.IsDesign}), its design matrix is positive definite and
$x^\top A(w)^{-1}x \le d$ for all $x \in \Xs$; \texttt{COLT83.exists\_isGOptimalDesign} is the
...
\lean{COLT83.IsGOptimalDesign.norm_sqrt_inv_le, COLT83.IsGOptimalDesign.norm_sqrt_inv_eq}\leanok
...
\lean{COLT83.normalizeMat, COLT83.IsGOptimalDesign.norm_normalizeMat_eq_one, COLT83.IsGOptimalDesign.designMatrix_mapDomain_normalizeMat, COLT83.IsGOptimalDesign.norm_normalizeMat_le, COLT83.IsGOptimalDesign.isDesign_mapDomain_normalizeMat, COLT83.IsGOptimalDesign.span_image_normalizeMat_eq_top, COLT83.isUnit_normalizeMat}\leanok
...
\lean{COLT83.exists_isDesign_of_mem_designSet, COLT83.mem_designSet_iff}\leanok
\uses{def:design_matrix}
...
$\texttt{EuclideanSpace ℝ ι →₀ ℝ}$ (\texttt{Finsupp}) with the predicate \texttt{COLT83.IsDesign}
(support in $\Xs$, nonnegative, summing to one), and $A(\lambda)$ is the linear map
\texttt{COLT83.designMatrix}; the equality of $\designset(\Xs)$ with the set of design matrices
of design distributions is \texttt{COLT83.mem\_designSet\_iff}.
...
\lean{COLT83.barrier, COLT83.barrier_eq_sum, COLT83.barrier_neg_card_zero}\leanok
\uses{lem:eigen_quadratic_bounds}
...
\lean{COLT83.barrierScore, COLT83.barrier_lt_barrier_add_half, COLT83.barrier_step}\leanok
...
\lean{COLT83.barrier_average}\leanok
...
\lean{COLT83.exists_rounding}\leanok
...
satisfies the invariant'' (\texttt{COLT83.exists\_rounding}); the greedy choice at each step is
the point provided by Lemma~\ref{lem:barrier_average} (\texttt{COLT83.barrier\_average}, stated
...
\lean{COLT83.posDef_of_smul_le, COLT83.dotProduct_inv_mulVec_le_of_smul_le, COLT83.gwMat_le_of_smul_le}\leanok
