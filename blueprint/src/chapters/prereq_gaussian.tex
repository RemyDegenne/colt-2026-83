...
\lean{ProbabilityTheory.multivariateGaussian_map_toEuclideanCLM, ProbabilityTheory.multivariateGaussian_map_toEuclideanLin, ProbabilityTheory.hasLaw_finset_sum_gaussianReal, ProbabilityTheory.hasLaw_const_add_const_mul_sum_pi_gaussianReal}
\uses{def:pg_gaussian_vector}
...
\textbf{Lean remark.} In the Lean development the pair of Gaussian vectors of
Lemmas~\ref{lem:gauss_joint_linear} and~\ref{lem:gauss_uncorrelated_indep} is always of the
form $(A_1g + A_2\eta,\ B_1g + B_2\eta)$ for a standard Gaussian pair $(g,\eta)$ of
$\R^{d}\times\R^{T}$ (\texttt{isGaussian\_prod\_stdGaussian}: the product of two standard
Gaussian measures is a Gaussian measure on the product space) and continuous linear maps
$A_i, B_i$; the cross-covariance is then
$\operatorname{Cov}(\ip{a}{A_1g+A_2\eta}, \ip{b}{B_1g+B_2\eta}) = \ip{a}{(A_1B_1^* + A_2B_2^*)b}$
(\texttt{covariance\_inner\_add\_prod\_stdGaussian}), and the independence criterion becomes
$A_1B_1^* + A_2B_2^* = 0$
(\texttt{indepFun\_add\_prod\_stdGaussian\_of\_comp\_adjoint\_eq\_zero}).

\begin{lemma}[Joint law of two linear images]\label{lem:gauss_joint_linear}
\lean{ProbabilityTheory.isGaussian_prod_stdGaussian, ProbabilityTheory.covariance_inner_add_prod_stdGaussian, ProbabilityTheory.map_toLp_prod_stdGaussian}\leanok
In Lean this is stated for the standard Gaussian measures of two arbitrary
finite-dimensional real inner product spaces $E$, $F$: their product is the standard Gaussian
measure of $E \times F$ with the inner product $\ip{(a,b)}{(x,y)} = \ip a x + \ip b y$.
\uses{def:pg_gaussian_vector}
...
\lean{ProbabilityTheory.indepFun_add_prod_stdGaussian_of_comp_adjoint_eq_zero}\leanok
\uses{def:pg_gaussian_vector}
...
\lean{ProbabilityTheory.IndepFun.integral_inner_comp_eq_zero, ProbabilityTheory.integrable_inner_comp}\leanok
...
\begin{proof}
\leanok
(The Lean proof is basis-free: the law of $(V, W)$ is the product of the marginals, so by
Fubini $\E\ip{f(V)}{W} = \int\ip{f(v)}{\E W}\,\Prob_V(dv) = 0$.)
...
\lean{ProbabilityTheory.integral_abs_gaussianReal}
\uses{def:pg_gaussian_vector}
...
(Lean: the absolute moment is \texttt{integral\_abs\_gaussianReal}, $\E\abs Z = \sqrt{2\sigma^2/\pi}$,
computed with the density and $\int_0^\infty xe^{-bx^2}\,dx = 1/(2b)$
(\texttt{integral\_Ioi\_mul\_exp\_neg\_mul\_sq}); the fourth moment is not formalized.)
\end{proof}
...
\begin{lemma}[Lower bound on the expected norm of a Gaussian vector]\label{lem:gauss_norm_lower}
\lean{ProbabilityTheory.integral_abs_eval_multivariateGaussian, ProbabilityTheory.sum_mul_sqrt_diag_le_integral_norm_multivariateGaussian}\leanok
\uses{def:pg_gaussian_vector, lem:gauss_1d_moments, lem:gauss_linear_image}
Let $S \in \PSD$ and $Y \sim \Normal(0,S)$. For every $c \in \R^d$ with $c_i \ge 0$ and
$\sum_i c_i^2 \le 1$,
\[
  \E\norm{Y} \;\ge\; \sum_i c_i\,\E\abs{Y_i} \;=\; \sqrt{2/\pi}\,\sum_i c_i\sqrt{S_{ii}} .
\]
\end{lemma}

\begin{proof}
\leanok
\uses{lem:gauss_1d_moments, lem:gauss_linear_image}
Pointwise, by Cauchy--Schwarz,
$\sum_i c_i\abs{y_i} \le (\sum_i c_i^2)^{1/2}(\sum_i y_i^2)^{1/2} \le \norm{y}$; integrate.
Each coordinate $Y_i$ has law $\Normal(0, S_{ii})$ (Lemma~\ref{lem:gauss_linear_image}), so
$\E\abs{Y_i} = \sqrt{2S_{ii}/\pi}$ by Lemma~\ref{lem:gauss_1d_moments}.
\end{proof}

\begin{lemma}[Gaussian tail]\label{lem:gauss_tail}
\lean{ProbabilityTheory.hasSubgaussianMGF_fun_id_gaussianReal}\leanok
\uses{def:pg_gaussian_vector}
...
\lean{ProbabilityTheory.integral_iSup_le_of_hasSubgaussianMGF, ProbabilityTheory.integrable_iSup_of_hasSubgaussianMGF, ProbabilityTheory.HasSubgaussianMGF.mono, ProbabilityTheory.hasSubgaussianMGF_inner_stdGaussian, ProbabilityTheory.integral_iSup_inner_stdGaussian_le, ProbabilityTheory.gaussianWidth_stdGaussian_le_of_finite}\leanok
\uses{lem:gauss_tail}
