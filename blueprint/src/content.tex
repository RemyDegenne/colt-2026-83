...
\begin{itemize}
  ...
  \item The width of the multi-task set (Theorem~\ref{thm:width_separation}) is computed in
        Lean without the Helmert matrices: a basis-free reduction bounds the intrinsic width of
        a non-spanning set by the expected supremum of a Gaussian vector supported on its span,
        whose covariance is the pseudo-inverse of a design matrix
        (Lemma~\ref{lem:multitask_width_reduction}); only the description of the span and
        the action of the uniform design on it are needed.
\end{itemize}
...
\textbf{Status.} The formalization proceeds in two phases. Phase~1 (done) states the headline
results of the paper in Lean, over definitions written in library generality: each such
statement carries a \verb+\lean+ tag naming its Lean declaration and is marked as formalized,
with its proof left to phase~2; the upper bounds are stated in the paper's existential form
(Corollaries~\ref{cor:upper_exists}, \ref{cor:log_gains_exists}, \ref{cor:separation_exists}
and~\ref{cor:norm_estimation_exists}), the explicit algorithms being part of the proofs. Each
headline statement is also a standalone challenge file for \texttt{leanprover/comparator}
(directory \texttt{comparator/}). Phase~2 will formalize the proofs, following this document.

\textbf{Blueprint author.} Rémy Degenne. Anyone is welcome to contribute!

\input{chapters/model.tex}
\input{chapters/design.tex}
\input{chapters/upper.tex}
\input{chapters/lower_nonadaptive.tex}
\input{chapters/lower_adaptive.tex}
\input{chapters/width.tex}
\input{chapters/log_gains.tex}
\input{chapters/norm_estimation.tex}
\input{chapters/separation.tex}
...
\input{chapters/prereq_gaussian.tex}
\input{chapters/prereq_concentration.tex}
\input{chapters/prereq_subexp.tex}
\input{chapters/prereq_info.tex}
\input{chapters/prereq_sudakov.tex}
\input{chapters/prereq_median.tex}
\input{chapters/prereq_bayes.tex}
