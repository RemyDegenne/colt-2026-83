...
restrictions to a measurable set and to its complement (`klDiv_restrict_add_restrict_compl`), so
that restricting both measures to a measurable set does not increase the divergence
(`klDiv_restrict_le`), and the divergence is the supremum of the divergences of the restrictions
to an increasing sequence of measurable sets covering the space (`klDiv_eq_iSup_restrict`,
a monotone convergence property).

The proofs go through the representation `klDiv μ ν = ∫⁻ x, klFun (∂μ/∂ν x) ∂ν` when `μ ≪ ν`,
and the fact that the Radon–Nikodym derivative of the restrictions is the restriction of the
Radon–Nikodym derivative (`Measure.rnDeriv_restrict_restrict`).
-/
...
lemma rnDeriv_restrict_restrict [SigmaFinite μ] [SigmaFinite ν] (hμν : μ ≪ ν) {s : Set α}
    (hs : MeasurableSet s) :
    (μ.restrict s).rnDeriv (ν.restrict s) =ᵐ[ν.restrict s] μ.rnDeriv ν := by
  have h : μ.restrict s = (ν.restrict s).withDensity (μ.rnDeriv ν) := by
    rw [← restrict_withDensity hs, withDensity_rnDeriv_eq _ _ hμν]
  rw [h]
  exact rnDeriv_withDensity _ (measurable_rnDeriv _ _)
...
/-- The divergence of the restrictions of `μ ≪ ν` to a measurable set `s`, as the integral of
`klFun (∂μ/∂ν)` over `s`. -/
lemma klDiv_restrict_of_ac (hμν : μ ≪ ν) (hs : MeasurableSet s) :
    klDiv (μ.restrict s) (ν.restrict s) =
      ∫⁻ x in s, ENNReal.ofReal (klFun (μ.rnDeriv ν x).toReal) ∂ν := by
  rw [klDiv_eq_lintegral_klFun_of_ac (hμν.restrict s)]
  refine lintegral_congr_ae ?_
  filter_upwards [Measure.rnDeriv_restrict_restrict hμν hs] with x hx
  rw [hx]
...
lemma klDiv_restrict_add_restrict_compl (hs : MeasurableSet s) :
  ...
  by_cases hμν : μ ≪ ν
  · rw [klDiv_restrict_of_ac hμν hs, klDiv_restrict_of_ac hμν hs.compl,
...
lemma klDiv_eq_iSup_restrict {s : ℕ → Set α} (hs : ∀ n, MeasurableSet (s n))
  ...
  by_cases hμν : μ ≪ ν
  · simp_rw [klDiv_restrict_of_ac hμν (hs _), klDiv_eq_lintegral_klFun_of_ac hμν,
