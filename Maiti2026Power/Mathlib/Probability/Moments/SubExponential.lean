...
Mathlib's `HasSubgaussianMGF` (see `Mathlib/Probability/Moments/SubGaussian.lean`, whose
...
* `hasSubexponentialMGF_of_abs_le_of_integral_eq_zero`: Bernstein's condition, a centered
  variable bounded by `M` with second moment at most `v` has parameters `(3 * v / 2, M)`.
...
arithmetic of the applications; the lemmas that need `0 < V` or `0 < b` assume it explicitly.
...
The set of admissible `t` is the closed interval `{t | b * |t| ≤ 1}` (all of `ℝ` when `b ≤ 0`).
In `Kernel.HasSubexponentialMGF.of_ae_mgf_le`, the bound on the mgf is upgraded from "for all
`t`, almost everywhere" to "almost everywhere, for all `t`" by continuity of the mgf on the
interior of that interval, the two possible endpoints being handled separately.
-/
...
/-! ### The admissible set `{t | b * |t| ≤ 1}` -/

section Admissible

variable {b b' t r : ℝ}

lemma mul_abs_le_one_iff (hb : 0 < b) : b * |t| ≤ 1 ↔ |t| ≤ 1 / b := by
  rw [le_div_iff₀ hb, mul_comm]

lemma mul_abs_le_one_of_nonpos (hb : b ≤ 0) (t : ℝ) : b * |t| ≤ 1 := by
  nlinarith [abs_nonneg t]

lemma mul_abs_one_div_le_one (hb : 0 < b) : b * |1 / b| ≤ 1 := by
  rw [abs_of_pos (by positivity), mul_one_div_cancel hb.ne']

lemma mul_abs_neg_one_div_le_one (hb : 0 < b) : b * |-(1 / b)| ≤ 1 := by
  rw [abs_neg]
  exact mul_abs_one_div_le_one hb

lemma mul_abs_inv_abs_add_one_le_one (b : ℝ) : b * |(|b| + 1)⁻¹| ≤ 1 := by
  rw [abs_of_pos (by positivity), mul_inv_le_iff₀ (by positivity), one_mul]
  linarith [le_abs_self b]

lemma mul_abs_le_one_of_le (hb : b ≤ b') (ht : b' * |t| ≤ 1) : b * |t| ≤ 1 :=
  (mul_le_mul_of_nonneg_right hb (abs_nonneg t)).trans ht

lemma mul_abs_mul_le_one (ht : |r| * b * |t| ≤ 1) : b * |t * r| ≤ 1 :=
  calc b * |t * r| = |r| * b * |t| := by rw [abs_mul]; ring
    _ ≤ 1 := ht

lemma eventually_mul_abs_le (b : ℝ) : ∀ᶠ t : ℝ in 𝓝 0, b * |t| ≤ 1 := by
...
structure Kernel.HasSubexponentialMGF (X : Ω → ℝ) (V b : ℝ)
  ...
  integrable_exp_mul : ∀ t : ℝ, b * |t| ≤ 1 → Integrable (fun ω ↦ exp (t * X ω)) (κ ∘ₘ ν)
  mgf_le : ∀ᵐ ω' ∂ν, ∀ t : ℝ, b * |t| ≤ 1 → mgf X (κ ω') t ≤ exp (V * t ^ 2 / 2)
...
lemma aestronglyMeasurable (h : HasSubexponentialMGF X V b κ ν) :
  ...
  have h_int := h.integrable_exp_mul _ (mul_abs_inv_abs_add_one_le_one b)
...
lemma ae_aestronglyMeasurable (h : HasSubexponentialMGF X V b κ ν) :
  ...
  filter_upwards [h.ae_integrable_exp_mul (mul_abs_inv_abs_add_one_le_one b)] with ω h_int
  exact (aemeasurable_of_aemeasurable_exp_mul (by positivity)
    h_int.1.aemeasurable).aestronglyMeasurable
...
lemma ae_forall_integrable_exp_mul_of_forall
  ...
  · have h (n : ℤ) : ∀ᵐ ω' ∂ν, Integrable (fun ω ↦ exp (n * X ω)) (κ ω') :=
      Measure.ae_integrable_of_integrable_comp (h_int _ (mul_abs_le_one_of_nonpos hb _))
    rw [← ae_all_iff] at h
    filter_upwards [h] with ω' h t _
    exact integrable_exp_mul_of_le_of_le (h _) (h _) (Int.floor_le t) (Int.le_ceil t)
  · filter_upwards [Measure.ae_integrable_of_integrable_comp (h_int _ (mul_abs_one_div_le_one hb)),
      Measure.ae_integrable_of_integrable_comp (h_int _ (mul_abs_neg_one_div_le_one hb))]
      with ω' h1 h2 t ht
    rw [mul_abs_le_one_iff hb, abs_le] at ht
  ...
  mgf_le := by
    ...
    have h_end : ∀ᵐ ω' ∂ν, ∀ t, b * |t| = 1 → mgf X (κ ω') t ≤ exp (V * t ^ 2 / 2) := by
      ...
      · refine ae_of_all _ fun _ t ht ↦ absurd ht ?_
        nlinarith [abs_nonneg t]
      · filter_upwards [h_mgf _ (mul_abs_one_div_le_one hb),
          h_mgf _ (mul_abs_neg_one_div_le_one hb)] with ω' h1 h2 t ht
...
lemma mono (h : HasSubexponentialMGF X V b κ ν) {V' b' : ℝ} (hV : V ≤ V') (hb : b ≤ b') :
  ...
  integrable_exp_mul t ht := h.integrable_exp_mul t (mul_abs_le_one_of_le hb ht)
  mgf_le := by
    filter_upwards [h.mgf_le] with ω' hω t ht
    refine (hω t (mul_abs_le_one_of_le hb ht)).trans ?_
    gcongr

lemma of_hasSubgaussianMGF {c : ℝ≥0} (h : Kernel.HasSubgaussianMGF X c κ ν) (b : ℝ) :
...
protected lemma const_mul (h : HasSubexponentialMGF X V b κ ν) (r : ℝ) :
  ...
  integrable_exp_mul t ht := by
    simp_rw [← mul_assoc]
    exact h.integrable_exp_mul (t * r) (mul_abs_mul_le_one ht)
  mgf_le := by
    filter_upwards [h.mgf_le] with ω hω t ht
    rw [mgf_const_mul, mul_comm r t]
    refine (hω (t * r) (mul_abs_mul_le_one ht)).trans_eq ?_
    congr 1
    ring
...
lemma measure_ge_le_exp_neg_sq (h : HasSubexponentialMGF X V b κ ν) (hV : 0 < V) {t : ℝ}
    (ht : 0 ≤ t) (htb : t * b ≤ V) :
...
lemma measure_ge_le_exp_neg_div (h : HasSubexponentialMGF X V b κ ν) (hb : 0 < b) {t : ℝ}
  ...
  filter_upwards [h.measure_ge_le_exp_add t] with ω' h
  refine (h (1 / b) (by positivity) (mul_abs_one_div_le_one hb)).trans ?_
  rw [exp_le_exp]
...
lemma measure_ge_le (h : HasSubexponentialMGF X V b κ ν) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
    (ht : 0 ≤ t) :
    ∀ᵐ ω' ∂ν, (κ ω').real {ω | t ≤ X ω} ≤ exp (-min (t ^ 2 / (2 * V)) (t / (2 * b))) := by
  rcases le_or_gt (t * b) V with htb | htb
  · filter_upwards [h.measure_ge_le_exp_neg_sq hV ht htb] with ω' h
...
lemma measure_le_le (h : HasSubexponentialMGF X V b κ ν) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
    (ht : 0 ≤ t) :
    ∀ᵐ ω' ∂ν, (κ ω').real {ω | X ω ≤ -t} ≤ exp (-min (t ^ 2 / (2 * V)) (t / (2 * b))) := by
  filter_upwards [h.neg.measure_ge_le hV hb ht] with ω' h
  simpa only [Pi.neg_apply, le_neg] using h

/-- **Bernstein-type tail bound**, two-sided. -/
lemma measure_abs_ge_le (h : HasSubexponentialMGF X V b κ ν) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
    (ht : 0 ≤ t) :
    ∀ᵐ ω' ∂ν, (κ ω').real {ω | t ≤ |X ω|}
      ≤ 2 * exp (-min (t ^ 2 / (2 * V)) (t / (2 * b))) := by
  filter_upwards [h.measure_ge_le hV hb ht, h.measure_le_le hV hb ht, h.isFiniteMeasure]
    with ω' h1 h2 _
  calc (κ ω').real {ω | t ≤ |X ω|}
...
lemma of_hasCondSubgaussianMGF {c : ℝ≥0} (h : HasCondSubgaussianMGF m hm X c μ) (b : ℝ) :
    HasCondSubexponentialMGF m hm X c b μ :=
  Kernel.HasSubexponentialMGF.of_hasSubgaussianMGF h b
...
lemma mono (h : HasSubexponentialMGF X V b μ) {V' b' : ℝ} (hV : V ≤ V') (hb : b ≤ b') :
  ...
  integrable_exp_mul t ht := h.integrable_exp_mul t (mul_abs_le_one_of_le hb ht)
  mgf_le t ht := by
    refine (h.mgf_le t (mul_abs_le_one_of_le hb ht)).trans ?_
    gcongr
...
lemma measure_ge_le_exp_neg_sq (h : HasSubexponentialMGF X V b μ) (hV : 0 < V) {t : ℝ}
  ...
  rw [hasSubexponentialMGF_iff_kernel] at h
  simpa using h.measure_ge_le_exp_neg_sq hV ht htb
...
lemma measure_ge_le (h : HasSubexponentialMGF X V b μ) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
  ...
  rw [hasSubexponentialMGF_iff_kernel] at h
  simpa using h.measure_ge_le hV hb ht
...
lemma measure_le_le (h : HasSubexponentialMGF X V b μ) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
  ...
  rw [hasSubexponentialMGF_iff_kernel] at h
  simpa using h.measure_le_le hV hb ht
...
lemma measure_abs_ge_le (h : HasSubexponentialMGF X V b μ) (hV : 0 < V) (hb : 0 < b) {t : ℝ}
  ...
  rw [hasSubexponentialMGF_iff_kernel] at h
  simpa using h.measure_abs_ge_le hV hb ht

lemma measure_abs_gt_le [IsFiniteMeasure μ] (h : HasSubexponentialMGF X V b μ) (hV : 0 < V)
    (hb : 0 < b) {t : ℝ} (ht : 0 ≤ t) :
    μ.real {ω | t < |X ω|} ≤ 2 * exp (-min (t ^ 2 / (2 * V)) (t / (2 * b))) := by
...
lemma measure_sum_ge_le_of_iIndepFun {ι : Type*} {X : ι → Ω → ℝ} (h_indep : iIndepFun X μ)
    (h_meas : ∀ i, Measurable (X i)) {V : ι → ℝ} {s : Finset ι}
    (h : ∀ i ∈ s, HasSubexponentialMGF (X i) (V i) b μ) (hV : 0 < ∑ i ∈ s, V i) (hb : 0 < b)
    {t : ℝ} (ht : 0 ≤ t) :
    μ.real {ω | t ≤ ∑ i ∈ s, X i ω}
      ≤ exp (-min (t ^ 2 / (2 * ∑ i ∈ s, V i)) (t / (2 * b))) := by
  have := h_indep.isProbabilityMeasure
  have := (sum_of_iIndepFun h_indep h_meas s h).measure_ge_le hV hb ht
...
lemma measure_sum_le_le_of_iIndepFun {ι : Type*} {X : ι → Ω → ℝ} (h_indep : iIndepFun X μ)
  ...
  simp_rw [le_neg (b := t), ← Finset.sum_neg_distrib, ← Pi.neg_apply (f := X _),
    ← Pi.neg_apply (f := X)]
  refine measure_sum_ge_le_of_iIndepFun (X := -X) (μ := μ) ?_ ?_ ?_ hV hb ht
  · exact h_indep.comp _ (fun _ ↦ measurable_neg)
  · exact fun i ↦ (h_meas i).neg
...
lemma measure_sum_range_ge_le_of_iIndepFun {X : ℕ → Ω → ℝ} (h_indep : iIndepFun X μ)
    (h_meas : ∀ i, Measurable (X i)) {n : ℕ} (h : ∀ i < n, HasSubexponentialMGF (X i) V b μ)
    (hV : 0 < V) (hb : 0 < b) {t : ℝ} (ht : 0 ≤ t) :
    μ.real {ω | t ≤ ∑ i ∈ Finset.range n, X i ω}
      ≤ exp (-min (t ^ 2 / (2 * n * V)) (t / (2 * b))) := by
  have := h_indep.isProbabilityMeasure
  rcases Nat.eq_zero_or_pos n with rfl | hn
  · simp only [CharP.cast_eq_zero, mul_zero, zero_mul, div_zero]
    rw [min_eq_left (by positivity), neg_zero, exp_zero]
    exact measureReal_le_one
  have h := measure_sum_ge_le_of_iIndepFun h_indep h_meas (V := fun _ ↦ V) (s := Finset.range n)
    (by simpa) (by rw [Finset.sum_const, Finset.card_range, nsmul_eq_mul]; positivity) hb ht
...
lemma measure_sum_range_le_le_of_iIndepFun {X : ℕ → Ω → ℝ} (h_indep : iIndepFun X μ)
  ...
  simp_rw [le_neg (b := t), ← Finset.sum_neg_distrib, ← Pi.neg_apply (f := X _),
    ← Pi.neg_apply (f := X)]
  refine measure_sum_range_ge_le_of_iIndepFun (X := -X) (μ := μ) ?_ ?_ ?_ hV hb ht
  · exact h_indep.comp _ (fun _ ↦ measurable_neg)
  · exact fun i ↦ (h_meas i).neg
  · exact fun i hi ↦ (h i hi).neg
...
lemma measureReal_le_le_exp {Y : Ω → ℝ} {VX VY : ℝ}
    ...
    (hindep : IndepFun X Y μ) (h_le : μ[Y] ≤ μ[X]) (hV : 0 < VX + VY) (hb : 0 < b) :
    μ.real {ω | X ω ≤ Y ω}
  ...
  _ ≤ exp (-min ((μ[X] - μ[Y]) ^ 2 / (2 * (VX + VY))) ((μ[X] - μ[Y]) / (2 * b))) :=
    hXY.measure_ge_le hV hb (by linarith)
...
/-- **Bernstein's condition** for bounded random variables: if `|X| ≤ M` almost surely, `X` is
centered and `μ[X ^ 2] ≤ v`, then `X` has a sub-exponential mgf with parameters
`(3 * v / 2, M)`. -/
lemma hasSubexponentialMGF_of_abs_le_of_integral_eq_zero [IsProbabilityMeasure μ] {M v : ℝ}
    (hm : AEMeasurable X μ) (hM : ∀ᵐ ω ∂μ, |X ω| ≤ M) (hc : μ[X] = 0)
    (hv : μ[fun ω ↦ X ω ^ 2] ≤ v) :
...
/-- **Bernstein's condition** for bounded random variables: if `|X| ≤ M` almost surely, then
`X - μ[X]` has a sub-exponential mgf with parameters `(3 * Var[X] / 2, 2 * M)`. -/
lemma hasSubexponentialMGF_sub_integral_of_abs_le [IsProbabilityMeasure μ] {M : ℝ}
  ...
  · rw [variance_eq_integral hm]

end BernsteinCondition
...
lemma HasSubexponentialMGF.sum_of_hasCondSubexponentialMGF [IsZeroOrProbabilityMeasure μ]
    (h_adapted : StronglyAdapted ℱ Y) (h0 : HasSubexponentialMGF (Y 0) (VY 0) b μ) (n : ℕ)
    (h_sub : ∀ i < n - 1,
...
/-- **Azuma–Bernstein inequality** for sums of conditionally sub-exponential random variables. -/
lemma measure_sum_ge_le_of_hasCondSubexponentialMGF [IsZeroOrProbabilityMeasure μ]
    (h_adapted : StronglyAdapted ℱ Y) (h0 : HasSubexponentialMGF (Y 0) (VY 0) b μ) (n : ℕ)
    (h_sub : ∀ i < n - 1,
      HasCondSubexponentialMGF (ℱ i) (ℱ.le i) (Y (i + 1)) (VY (i + 1)) b μ)
    (hV : 0 < ∑ i ∈ Finset.range n, VY i) (hb : 0 < b) {t : ℝ} (ht : 0 ≤ t) :
    μ.real {ω | t ≤ ∑ i ∈ Finset.range n, Y i ω}
      ≤ exp (-min (t ^ 2 / (2 * ∑ i ∈ Finset.range n, VY i)) (t / (2 * b))) :=
  (HasSubexponentialMGF.sum_of_hasCondSubexponentialMGF h_adapted h0 n h_sub).measure_ge_le
    hV hb ht
