...
of the seed. It is an LML algorithm (`SeededAlg.toAlgorithm`) whose policy at round `n` is the
kernel `h ↦ seed.map (next n h)`.
...
lemma IsAlgEnvSeq.map_history_eq [IsProbabilityMeasure P] [IsProbabilityMeasure P']
    (h : IsAlgEnvSeq X Y alg env P) (h' : IsAlgEnvSeq X' Y' alg env P') (n : ℕ) :
    P.map (history X Y n) = P'.map (history X' Y' n) := by
  induction n with
  | zero => rw [(hasLaw_history_zero X Y).map_eq, (hasLaw_history_zero X' Y').map_eq]
...
noncomputable def policy (n : ℕ) : Kernel (Fin n → 𝓐 × 𝓨) 𝓐 :=
  Kernel.mapOfConst A.seed (A.next n) (A.measurable_next n)

lemma policy_apply (n : ℕ) (h : Fin n → 𝓐 × 𝓨) : A.policy n h = A.seed.map (A.next n h) :=
...
@[simp] lemma p0_toAlgorithm : A.toAlgorithm.p0 = A.seed.map (A.next 0 default) := by
  rw [Algorithm.p0_def, policy_toAlgorithm, policy_apply]
...
lemma history_seed (T : ℕ) :
    history (A.seedAction F) (A.seedFeedback F) T = A.seedFinHist F T := rfl
...
lemma measurable_seedStep (hF : Measurable (Function.uncurry F)) (n : ℕ) :
  ...
  | _ n ih =>
    ...
    have hh : Measurable fun ω ↦ (fun i : Fin n ↦ A.seedStep F i ω) :=
      measurable_pi_lambda _ fun i ↦ ih i i.2
...
noncomputable def seedHist (n : ℕ) (w : Fin n → U × E) : Fin n → 𝓐 × 𝓨 :=
...
lemma hasCondDistrib_coord (n : ℕ) :
  ...
  exact h.comp (measurable_pi_lambda _ fun i : Fin n ↦
...
lemma isAlgEnvSeq_seed :
  ...
  hasCondDistrib_feedback n := by
    ...
    have hk : Kernel.mapOfConst μe (fun (q : (Fin n → U × E) × U) (e : E) ↦
        ...
        = ((noiseKernel μe F hF).prodMkLeft (Fin n → 𝓐 × 𝓨)).comap _ hf := by
      ext q : 1
    ...
    have e2 : ((fun q : (Fin n → U × E) × U ↦
        ...
        = fun ω ↦ (history (A.seedAction F) (A.seedFeedback F) n ω, A.seedAction F n ω) := by
      funext ω
      simp only [Function.comp_apply, A.seedHist_seedPrefix F μe n ω]
      congr 1
...
lemma _root_.Learning.IsAlgEnvSeq.map_history_eq_seed {Ω : Type*} [MeasurableSpace Ω]
    ...
    P.map (history X Y T) = (A.seedMeasure μe).map (A.seedFinHist F T) :=
...
lemma _root_.Learning.IdentAlg.IsRun.measureReal_eq_seed {𝓞 : Type*} [MeasurableSpace 𝓞]
  ...
  calc P.real {ω | out ω ∈ s}
      = P.real {ω | g (history X Y T ω) ∈ s} := by
        apply measureReal_congr
        filter_upwards [hae] with ω hω
        change (out ω ∈ s) = (g (history X Y T ω) ∈ s)
        rw [hω]
    _ = (P.map (history X Y T)).real (g ⁻¹' s) := by
