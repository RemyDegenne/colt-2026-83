...
lemma refl (alg : Algorithm 𝓐 𝓨) (N : ℕ) : alg.AgreeUntil alg N := ⟨fun _ _ ↦ rfl⟩
...
/-- If `(X, Y)` is an algorithm-environment sequence for `A.alg` until time `T` and `out` has
...
lemma map_history_out_eq [IsProbabilityMeasure P] [IsMarkovKernel (A.output T)]
    (h : IsAlgEnvSeqUntil X Y A.alg env P T)
    (hout : HasCondDistrib out (history X Y T) (A.output T) P) :
    P.map (fun ω ↦ (history X Y T ω, out ω)) =
      (A.fixedBudgetRunMeasure env T).map
        (fun ω ↦ (history IT.action IT.feedback T ω.1, ω.2)) := by
  ...
  congr 1
  rw [h.map_history]
  calc (trajMeasure A.alg env).map (history IT.action IT.feedback T)
      = ((trajMeasure A.alg env ⊗ₘ (A.output T).comap
          (history IT.action IT.feedback T) (measurable_history_traj T)).fst).map
          (history IT.action IT.feedback T) := by
...
`(X, Y)` is an algorithm-environment sequence for `A.alg` until time `T` in `env θ` and `out` has
...
lemma IsPAC.le_measureReal_of_isAlgEnvSeqUntil [IsProbabilityMeasure P] {Θ : Type*}
    ...
    (h : IsAlgEnvSeqUntil X Y A.alg (env θ) P T)
    (hout : HasCondDistrib out (history X Y T) (A.output T) P)
