...
| File | Result | Notes |
|---|---|---|
| `InformationTheory/KLBer.lean`, `InformationTheory/Pinsker.lean`, `InformationTheory/BretagnolleHuber.lean` | The binary divergences `klBerReal` (real) and `klBer` (`ℝ≥0∞`) and their basic properties; Pinsker's inequality for one event (through the Bernoulli case), the Bretagnolle–Huber inequality | Absent from Mathlib although `klDiv` is there. Reviewed for upstreaming on 2026-09-10: `KLBer.lean` needs only `Real.log` plus `Real.mul_log_div` (`Analysis/SpecialFunctions/Log/Basic.lean`, one lemma to put next to `Real.log_div`). `Pinsker.lean` needs `Probability/Distributions/Bernoulli.lean` (atoms, Lebesgue integral, density and absolute continuity of `bernoulliMeasure`, and the `unitInterval → ℝ≥0∞` coercion lemma that SetBernoulli's TODO asks for), which adds no import to Mathlib's file; the Radon–Nikodym lemma lives in `Pinsker.lean` to keep it that way. After the rebase, replace `map_mem_eq_bernoulliMeasure` by master's `hasLaw_indicator_one_bernoulliMeasure`. Natural Mathlib paths: `InformationTheory/KullbackLeibler/{Bernoulli,Pinsker}.lean`. Bretagnolle–Huber is independent and can be its own PR; reviewed on 2026-09-10 too (statement normalised to `exp (-KL) / 2`, the change of measure now goes through `Measure.lintegral_rnDeriv_mul`); its docstring cites `bretagnolle1979`, `tsybakov2009introduction` and `lattimore2020bandit`, none of which is in Mathlib's `docs/references.bib` yet. |
...
* `IdentAlg.lean`, `Run.lean`, `FixedBudget.lean`, `AlgorithmPrefix.lean`: since 2026-09-13 the identification algorithms themselves (structure, stopping time, stopped history, runs, output law, PAC property) are LML's `SequentialLearning/IdentificationAlg.lean`; still local: fixed budgets and fixed designs, the canonical run `IdentAlg.runMeasure` of every algorithm with `IsPAC.of_forall_isRun`, transport of runs, the stopped-history layer on `sigmaHistory` and data processing. This is the core abstraction of the project.
