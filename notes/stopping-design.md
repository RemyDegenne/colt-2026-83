...
**Status.** Step 4 below (Markov output kernels in this project) was done on 2026-09-11. On 2026-09-13 LML gained its own `IdentAlg` (`SequentialLearning/IdentificationAlg.lean`, with `SequentialLearning/StoppedHistory.lean`: `sigmaHistory`, `stoppedHistMeasure`) and this project switched to it: the structure has a `stopSet : Set (Σ n, Hist 𝓞 𝓐 𝓨 n)` field and a single output kernel on histories of variable length (no `stop n h` predicate, no per-length `output n`), `stoppingTime`/`stoppedHist` are Mathlib's `hittingAfter`/`stoppedValue` of `sigmaHistory`, and `IsPAC` is stated on the law of the output (`outputMeasure`) with a *badness* predicate and no universe parameter; the run-based form and the canonical run (Section 3.5) are this project's `Run.lean`. The general stopping layer (Section 3.1) and the change-of-measure results (3.7) are still local, restated on LML's objects. Everything else is a proposal.
